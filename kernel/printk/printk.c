...
static size_t print_time(u64 ts, char *buf)
{
	...
	rem_nsec = do_div(ts, 1000000000U);
...
skip:
		...
		if ((current_time - con_dura_time) > 2000000000ULL) {
			...
			rem_nsec_con_write_ttyS = do_div
				(time_con_write_ttyS, 1000000000U);
			rem_nsec_con_write_pstore = do_div
				(time_con_write_pstore, 1000000000U);
			tmp_rem_nsec_start = do_div(con_dura_time, 1000000000U);
			tmp_rem_nsec_end = do_div(current_time, 1000000000U);
...
"cpu%d [%lu.%06lu]--[%lu.%06lu] 'ttyS' %lubytes %lu.%06lus, 'pstore' %lubytes %lu.%06lus\n",
		...
		if (flag_toomuch == true) {
			...
			if (add_len + 12 <= 63) {
				period = delta_time;
				rem_nsec = do_div(period, 1000000000U);
