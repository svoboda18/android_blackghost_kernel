...
#ifdef CONFIG_MTK_SCHED_INTEROP
static int mt_sched_interop_rt(struct cpumask *lowest_mask,
	struct task_struct *p)
{
	...
	const struct cpumask *cpus_masks[5] = {0, 0, 0, 0, 0};

	if (pod_is_ready()) {
		for_each_perf_domain_ascending(domain) {
			tmp_domain[domain_cnt] = domain;
			cpus_masks[domain_cnt] = &domain->possible_cpus;
			domain_cnt++;
		}
	}
	// It is not a asymmetric CPU if domain_cnt is 0
	if (domain_cnt == 0) {
		cpus_masks[domain_cnt] = cpu_possible_mask;
		domain_cnt++;
	}

	for (i = 0; i < domain_cnt; i++) {
		domain = tmp_domain[i];
		cpumask_and(&cpus_mask, cpus_masks[i], lowest_mask);
...
static int find_lowest_rq(struct task_struct *task)
{
	...
	interop_cpu = mt_sched_interop_rt(lowest_mask, task);
	if (interop_cpu != -1)
		return interop_cpu;
#endif
