...
static int mtk_afe_mrgrx_awb_probe(struct snd_soc_component *component);
...
static int mtk_mrgrx_awb_pcm_copy(struct snd_pcm_substream *substream,
				  ...
				  void __user *dst, snd_pcm_uframes_t bytes)
{
	...
	PRINTK_AUD_AWB("%s  pos = %lu bytes = %lu\n ", __func__, pos, bytes);
	...
	if (Mrgrx_Awb_Capture_dma_buf == NULL) {
		...
		if (Mrgrx_Awb_Capture_dma_buf != NULL) {
			Mrgrx_Awb_Capture_dma_buf->area =
				dma_alloc_coherent(component->dev,
						   MRGRX_MAX_BUFFER_SIZE,
						   &Mrgrx_Awb_Capture_dma_buf->
						   addr, GFP_KERNEL);
