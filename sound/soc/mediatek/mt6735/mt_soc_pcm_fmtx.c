...
static int mtk_afe_fmtx_probe(struct snd_soc_component *component);
			     ...
			     void __user *dst, unsigned long bytes)
{
	...
	PRINTK_AUD_FMTX("[mtk_pcm_fmtx_copy] bytes = %lu bytes = %lu\n ", pos, bytes);
	...
	if (copy_size != 0) {
		...
		if (Afe_WriteIdx_tmp + copy_size < Afe_Block->u4BufferSize) { /* copy once */
			...
			PRINTK_AUD_FMTX("[%s] finish1, size:%x, WriteIdx:%x, ReadIdx=%x, Remained:%x, bytes=%x \r\n",
					__func__, copy_size, Afe_Block->u4WriteIdx, Afe_Block->u4DMAReadIdx,
					Afe_Block->u4DataRemained, bytes);
...
static struct snd_soc_component_driver mtk_fmtx_soc_platform = {
	.ops        = &mtk_fmtx_ops,
