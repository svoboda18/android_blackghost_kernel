...
static int mtk_afe_hdmi_probe(struct snd_soc_component *component);
			     ...
			     void __user *dst, unsigned long bytes)
{
	...
	PRINTK_AUD_HDMI("[mtk_pcm_hdmi_copy] bytes = %d WriteIdx=%x, ReadIdx=%x, DataRemained=%x\n",
			(kal_uint32)bytes, Afe_Block->u4WriteIdx, Afe_Block->u4DMAReadIdx,
...
static struct snd_soc_component_driver mtk_hdmi_soc_platform = {
	.ops        = &mtk_hdmi_ops,
...
static int mtk_afe_hdmi_probe(struct snd_soc_component *component)
{
	...
	HDMI_dma_buf->area = dma_alloc_coherent(component->dev,
