...
static int mtk_afe_capture2_probe(struct snd_soc_component *component);
				 ...
				 void __user *dst, unsigned long bytes)
{
	...
	PRINTK_AUD_UL2("mtk_capture2_pcm_copy pos = %lubytes = %lu\n ", pos, bytes);
...
static int mtk_afe_capture2_probe(struct snd_soc_component *component)
{
	...
	AudDrv_Allocate_mem_Buffer(component->dev, Soc_Aud_Digital_Block_MEM_VUL_DATA2, UL2_MAX_BUFFER_SIZE);
