...
static int mtk_asoc_dl1bt_probe(struct snd_soc_component *component);
			      ...
			      void __user *dst, unsigned long bytes)
{
	...
	PRINTK_AUD_DL1("mtk_pcm_dl1bt_copy pos = %lu bytes = %lu\n ", pos, bytes);
	...
	if (copy_size != 0) {
		...
		if (Afe_WriteIdx_tmp + copy_size < Afe_Block->u4BufferSize) { /* copy once */
			...
			PRINTK_AUD_DL1("%s finish1, size:%x, WriteIdx:%x, ReadIdx=%x, Remained:%x, bytes=%lu \r\n",
				       __func__, copy_size, Afe_Block->u4WriteIdx,
				       Afe_Block->u4DMAReadIdx, Afe_Block->u4DataRemained, bytes);
...
static struct snd_soc_component_driver mtk_soc_dl1bt_platform = {
	.ops        = &mtk_d1lbt_ops,
...
static int mtk_asoc_dl1bt_probe(struct snd_soc_component *component)
{
	...
	AudDrv_Allocate_mem_Buffer(component->dev, bt_dl_mem_blk, Dl1_MAX_BUFFER_SIZE);
	Dl1_Playback_dma_buf =  Get_Mem_Buffer(bt_dl_mem_blk);
