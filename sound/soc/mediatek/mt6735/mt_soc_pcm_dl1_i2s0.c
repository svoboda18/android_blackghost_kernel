...
static int mtk_afe_i2s0_probe(struct snd_soc_component *component);
			     ...
			     void __user *dst, unsigned long bytes)
{
	...
	/* struct snd_pcm_runtime *runtime = substream->runtime; */
	PRINTK_AUD_DL1("%s  pos = 0x%x bytes = 0x%x\n ", __func__, (unsigned int) pos, (unsigned int)bytes);
	...
	PRINTK_AUD_DL1("copy_size=0x%x, bytes=0x%x\n", (unsigned int)copy_size, (unsigned int)bytes);
	...
	if (copy_size != 0) {
		...
		if (Afe_WriteIdx_tmp + copy_size < Afe_Block->u4BufferSize) { /* copy once */
			...
			PRINTK_AUD_DL1(" finish1, copy_size:%x, WriteIdx:%x, ReadIdx=%x, Remained:%x, bytes=%x \r\n",
...
static struct snd_soc_component_driver mtk_i2s0_soc_platform = {
	.ops        = &mtk_i2s0_ops,
