...
static int mtk_asoc_bt_dai_probe(struct snd_soc_component *component);
			       ...
			       void __user *dst, unsigned long bytes)
{
	...
	if (bytes >  Dai_Block->u4DataRemained)
	...
	return audio_bytes_to_frame(substream, bytes);
}
...
static struct snd_soc_component_driver mtk_bt_dai_soc_platform = {
	.ops        = &mtk_bt_dai_ops,
...
static int mtk_bt_dai_probe(struct platform_device *pdev)
{
	...
	return snd_soc_register_component(&pdev->dev,
					 &mtk_bt_dai_soc_platform, NULL, 0);
}
...
static int mtk_asoc_bt_dai_probe(struct snd_soc_component *component)
{
	...
	AudDrv_Allocate_mem_Buffer(component->dev, Soc_Aud_Digital_Block_MEM_DAI, BT_DAI_MAX_BUFFER_SIZE);
