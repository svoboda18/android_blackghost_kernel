...
static int mtk_afe_dl1_awb_probe(struct snd_soc_component *component);
				...
				void __user *dst, unsigned long bytes)
{
	...
	if (bytes >  Awb_Block->u4DataRemained)
	...
	PRINTK_AUD_AWB("%s finish0, bytes:0x%x, read_size:0x%x, DataRemained:0x%x, ReadIdx:0x%x, WriteIdx:0x%x \r\n",
		       __func__, bytes, read_size, Awb_Block->u4DataRemained,
		       Awb_Block->u4DMAReadIdx, Awb_Block->u4WriteIdx);
...
static int mtk_afe_dl1_awb_probe(struct snd_soc_component *component)
{
	...
	AudDrv_Allocate_mem_Buffer(component->dev, Soc_Aud_Digital_Block_MEM_AWB, AWB_MAX_BUFFER_SIZE);
