...
static int mtk_asoc_dl2_probe(struct snd_soc_component *component);
...
static int mtk_pcm_dl2_copy_(void __user *dst, unsigned long *size, AFE_BLOCK_T *Afe_Block, bool bCopy)
{
			...
			void __user *dst, unsigned long bytes)
{
	...
	PRINTK_AUD_DL2("%s pos = %lu bytes = %lu, BufferSize %d, ConsumeSize %d\n", __func__, pos,
		bytes, ISRCopyBuffer.u4BufferSize, ISRCopyBuffer.u4IsrConsumeSize);
...
retry:
	if (ISRCopyBuffer.u4IsrConsumeSize <= 0) {
		...
		if (unlikely(ISRCopyBuffer.u4BufferSize < bytes))
			bytes = ISRCopyBuffer.u4BufferSize;

		/* for debug */
		detectData((char *)ISRCopyBuffer.pBufferIndx, bytes);

		ret = mtk_pcm_dl2_copy_((void *)ISRCopyBuffer.pBufferIndx, &bytes, Afe_Block, true);

		ISRCopyBuffer.pBufferIndx += bytes;
		ISRCopyBuffer.u4BufferSize -= bytes;
	} else {
		if (unlikely(ISRCopyBuffer.u4IsrConsumeSize < bytes)) {
			remainCount = bytes - ISRCopyBuffer.u4IsrConsumeSize;
			bytes = ISRCopyBuffer.u4IsrConsumeSize;
		}

		ret = mtk_pcm_dl2_copy_((void *)ISRCopyBuffer.pBufferIndx, &bytes, Afe_Block, false);
		ISRCopyBuffer.u4IsrConsumeSize -= bytes;
		if (ISRCopyBuffer.u4IsrConsumeSize < 0)
			ISRCopyBuffer.u4IsrConsumeSize = 0;

		if (unlikely(remainCount)) {
			if ((++retryCount) > 1) {
				pr_debug("%s, retryCount %d, remainCount %d, bytes %d\n", __func__,
					retryCount, remainCount, (int)bytes);
				if (retryCount > 5)
					goto exit;
			}
			bytes = remainCount;
			...
			void __user *dst, unsigned long bytes)
{
	...
	PRINTK_AUD_DL2("mtk_pcm_dl2_copy pos = %lu bytes = %lu\n", pos, bytes);

	return mtk_pcm_dl2_copy_(dst, &bytes, Afe_Block, true);
}
...
static int mtk_asoc_dl2_probe(struct snd_soc_component *component)
{
	...
	/* allocate dram */
	AudDrv_Allocate_mem_Buffer(component->dev, Soc_Aud_Digital_Block_MEM_DL2,
				   Dl2_MAX_BUFFER_SIZE);
