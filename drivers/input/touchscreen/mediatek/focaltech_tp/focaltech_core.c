...
#if FTS_PSENSOR_EN
extern struct alsps_init_info ps_init_info;
...
#if FTS_PSENSOR_EN
	if((ps_flag_sunritel == 1) && (ps_ctrl_flag_sunritel == 1)){
           ps_ctrl_flag_sunritel = 0;
           fts_enter_proximity_mode_sunritel(1);
	}
#endif
#if FTS_PSENSOR_EN
...
#if defined(CONFIG_MTK_HARDWAREINFO)
	fts_i2c_read_reg(client,FTS_REG_FW_VER,&FW_VER);
	fts_i2c_read_reg(client,FTS_REG_VENDOR_ID,&VENDOR_ID);
	printk("TP INFO FW_VER = %d  VENDOR_ID = %d\n",FW_VER,VENDOR_ID);
	printk("1111111111111111111111111\n");		
	g_ctp_fwvr_info = FW_VER;
	g_ctp_vendor_info = VENDOR_ID;
#endif	
    FTS_FUNC_EXIT();
    return 0;
