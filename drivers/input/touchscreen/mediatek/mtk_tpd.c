...
static int tpd_create_attributes(struct device *dev, struct tpd_attrs *attrs)
{
	int num = attrs->num;
	int ret = 0;

	for (; num > 0;) {
		ret = device_create_file(dev, attrs->attr[--num]);
		if (ret < 0) {
			pr_info("device_create_file error");
			break;
		}
	}
...
/*for some projects, we do not use mtk framebuffer*/
	...
	if (g_tpd_drv->attrs.num) {
		if (tpd_create_attributes(&pdev->dev, &g_tpd_drv->attrs) < 0) {
			pr_info("tpd_create_attributes failed");
			return -ENODEV;
		}
	}
