...
#define GT9XX_FW_NAME      CONFIG_GTP_FIRMWARE
#define GT9XX_FW_RETRY_TIME	20

#ifdef CONFIG_GTP_HOTKNOT
#define GT9XX_HN_AUTH_FW_NAME CONFIG_GTP_HOTKNOT_AUTH_FW
#define GT9XX_HN_TRANS_FW_NAME CONFIG_GTP_HOTKNOT_TRANS_FW
#endif
...
static u8 gup_check_update_file(struct i2c_client *client, struct st_fw_head *fw_head)
{
	s32 ret = 0;
	s32 i = 0;
	s32 fw_checksum = 0;
	...
	GTP_INFO("%s Request firmware...", __func__);
	for (i = 0; i < GT9XX_FW_RETRY_TIME; i++) {
		ret = request_firmware(&update_msg.fw, GT9XX_FW_NAME, &client->dev);
		if (!ret || (ret != -EAGAIN))
			break;
	}
	if (ret < 0) {
		GTP_ERROR("Request firmware failed - %s (%d), retry %d\n", GT9XX_FW_NAME, ret, i);
		return FAIL;
	}
			   ...
			   u16 chk_length)
{
	...
	memset(rd_buf, 0, PACK_SIZE + GTP_ADDR_LENGTH);
...
static u8 gup_burn_fw_boot(struct i2c_client *client)
{
	...
	ret = gup_load_section_file(fw_boot, (4 * FW_SECTION_LENGTH + FW_DSP_LENGTH),
...
static u8 gup_burn_fw_app_code(struct i2c_client *client)
{
	...
	start_index = 4 * FW_SECTION_LENGTH + FW_DSP_LENGTH +
...
#ifdef CONFIG_GTP_COMPATIBLE_MODE
	if (CHIP_TYPE_GT9F == gtp_chip_type) {
		ret = gup_fw_download_proc(dir, GTP_FL_FW_BURN);
		if (FAIL == ret)
			return ret;
...
static u8 gup_check_update_file_fl(struct i2c_client *client, struct st_fw_head *fw_head,
				   char *path)
{
	s32 i = 0;
	s32 fw_checksum = 0;
...
s32 gup_fw_download_proc(void *dir, u8 dwn_mode)
{
	...
	GTP_INFO("%s Request firmware...", __func__);
	for (i = 0; i < GT9XX_FW_RETRY_TIME; i++) {
		ret = request_firmware(&update_msg.fw, GT9XX_FW_NAME, &i2c_client_point->dev);
		if (!ret || (ret != -EAGAIN))
			break;
	}
	if (ret < 0) {
	...
	if (gtp_chip_type == CHIP_TYPE_GT9F)
		fw_size = FW_HEAD_LENGTH + FW_SECTION_LENGTH * 4 + FW_DSP_LENGTH;
	else
		fw_size = FW_HEAD_LENGTH + FW_SECTION_LENGTH * 4 +
			FW_DSP_ISP_LENGTH + FW_DSP_LENGTH + FW_BOOT_LENGTH;

	if (update_msg.fw->size < fw_size) {
		GTP_ERROR("INVALID firmware!");
		goto err_fw;
	}
...
s32 gup_load_hotknot_fw(void)
{
	...
	GTP_INFO("%s Request firmware...", __func__);
	for (i = 0; i < GT9XX_FW_RETRY_TIME; i++) {
		ret = request_firmware(&update_msg.fw, GT9XX_HN_TRANS_FW_NAME, &i2c_client_point->dev);
		if (!ret || (ret != -EAGAIN))
			break;
	}
	if (ret < 0) {
...
gup_load_hotknot_fw_exit:
	release_firmware(update_msg.fw);
	update_msg.fw = NULL;
	GTP_INFO("gup_load_hotknot transfer fw end.");
...
s32 gup_recovery_touch(void)
{
	...
	GTP_INFO("[gup_recovery_touch] Recovery to main fw.");
