...
static s32 tool_i2c_read_no_extra(u8 *buf, u16 len)
{
	...
	ret = gtp_tool_i2c_read(gt_client, buf, len + GTP_ADDR_LENGTH);
	return ret;
}
...
static s32 tool_i2c_write_no_extra(u8 *buf, u16 len)
{
	...
	ret = gtp_tool_i2c_write(gt_client, buf, len);
	return ret;
}
...
Return write length.
********************************************************/
static ssize_t goodix_tool_write(struct file *filp, const char __user *buff, size_t len,
				 loff_t *off)
{
	s32 ret = 0;
	ssize_t ret_len;
	...
	if (tpd_halt) {
		GTP_ERROR("touch is powered off!");
		return FAIL;
	}
	...
	if (1 == cmd_head.wr) {
		...
		ret_len = cmd_head.data_len + CMD_HEAD_LENGTH;
		goto exit;
		...
		wait_hotknot_timeout =
		    (cmd_head.data[GTP_ADDR_LENGTH + 1] << 8) + cmd_head.data[GTP_ADDR_LENGTH + 2];
		GTP_DEBUG
...
static ssize_t goodix_tool_read(struct file *flie, char __user *page, size_t size, loff_t *ppos)
{
	...
	if (cmd_head.wr % 2) {
		...
		ret =
		    simple_read_from_buffer(page, size, ppos, GTP_DRIVER_VERSION,
...
static ssize_t hotknot_write(struct file *filp, const char __user *buff, size_t len, loff_t *ppos)
{
	...
	if (1 == cmd_head2.wr) {
		...
		ret =
		    copy_from_user(&cmd_head2.data[GTP_ADDR_LENGTH], &buff[CMD_HEAD_LENGTH],
				   cmd_head2.data_len);

		if (ret)
			GTP_ERROR("copy_from_user failed.");

		memcpy(&cmd_head2.data[GTP_ADDR_LENGTH - cmd_head2.addr_len], cmd_head2.addr,
		       cmd_head2.addr_len);
		...
		ret_len = cmd_head2.data_len + CMD_HEAD_LENGTH;
		goto exit;
		...
		ret =
		    copy_from_user(&cmd_head2.data[0], &buff[CMD_HEAD_LENGTH], cmd_head2.data_len);
		...
		wait_hotknot_state |= rqst_hotknot_state;
		wait_hotknot_timeout =
		    (cmd_head2.data[GTP_ADDR_LENGTH + 1] << 8) + cmd_head2.data[GTP_ADDR_LENGTH +
										2];
		GTP_DEBUG
		    ("Goodix tool received wait polling state:0x%x,timeout:%d, all wait state:0x%x",
		     rqst_hotknot_state, wait_hotknot_timeout, wait_hotknot_state);
