...
struct port_proxy *port_proxy_alloc(int md_id, int md_capability, int napi_queue_mask, void *md)
{
	...
	ret = snprintf(proxy_p->wakelock_name, sizeof(proxy_p->wakelock_name), "md%d_wakelock", md_id + 1);
	if (ret < 0 || ret >= sizeof(proxy_p->wakelock_name)) {
		CCCI_ERROR_LOG(-1, TAG, "[%s] snprintf return error code :%d\n", __func__, ret);
		kfree(proxy_p);
		return NULL;
	}

	//wakeup_source_init(&proxy_p->wakelock, proxy_p->wakelock_name);
	proxy_p->wakelock = wakeup_source_register(NULL, proxy_p->wakelock_name);
