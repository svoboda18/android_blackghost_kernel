...
static void mdee_output_debug_info_to_buf(struct md_ee *mdee, DEBUG_INFO_T *debug_info, char *ex_info)
{
	...
	case MD_EX_CC_C2K_EXCEPTION:
		...
		if (debug_info->fatal_error.err_code1 == 0x3104) {
			...
			ret = snprintf(ex_info_temp, EE_BUF_LEN_UMOLY, "%s%s, MD base = 0x%08X\n\n", ex_info,
			...
			ret = snprintf(ex_info, EE_BUF_LEN_UMOLY, "%s", ex_info_temp);
			if (ret < 0 || ret >= EE_BUF_LEN_UMOLY)
				CCCI_ERROR_LOG(md_id, KERN,
					"[%s]-%d: snprintf fail, ret = %d\n",
					__func__, __LINE__, ret);
...
static void mdee_info_dump_v2(struct md_ee *mdee)
{
	...
	ex_info_buf = kmalloc(EE_BUF_LEN_UMOLY, GFP_ATOMIC);
	if (ex_info_buf == NULL) {
		CCCI_ERROR_LOG(md_id, KERN, "Fail alloc Mem for ex_info_buf!\n");
		goto err_exit;;
	}
	...
	for (core_id = 0; core_id < dumper->ex_core_num; core_id++) {
		if (core_id == 1)
			...
			ret = snprintf(ex_info_buf, EE_BUF_LEN_UMOLY, "%smd%d:%s", ex_info_temp, md_id + 1, ex_info);
			if (ret < 0 || ret >= EE_BUF_LEN_UMOLY)
				CCCI_ERROR_LOG(md_id, KERN,
						"[%s]-%d: snprintf fail,ret = %d\n",
						__func__, __LINE__, ret);

			ret = snprintf(ex_info_temp, EE_BUF_LEN_UMOLY, "%s", ex_info_buf);
			if (ret < 0 || ret >= EE_BUF_LEN_UMOLY)
				CCCI_ERROR_LOG(md_id, KERN,
						"[%s]-%d: snprintf fail,ret = %d\n",
						__func__, __LINE__, ret);
	...
	if (dumper->ex_core_num > 1) {
		...
		ret = snprintf(ex_info, EE_BUF_LEN_UMOLY, "%s", ex_info_buf);
		if (ret < 0 || ret >= EE_BUF_LEN_UMOLY)
			CCCI_ERROR_LOG(md_id, KERN,
					"[%s]-%d: snprintf fail,ret = %d\n",
					__func__, __LINE__, ret);
...
static int mdee_md32_core_parse(int md_id, DEBUG_INFO_T *debug_info, EX_MD32_LOG_T *ex_md32LogInfo)
{
	int ee_type = 0;
	int ee_case = 0;
	char temp_buf[MD_CORE_NAME_DEBUG];
	int ret = 0;
	...
	case 1:
		ret = snprintf(temp_buf, MD_CORE_NAME_DEBUG, "%s%s",
		...
		ret = snprintf(debug_info->core_name, MD_CORE_NAME_DEBUG,
					"%s", temp_buf);
		if (ret < 0 || ret >= MD_CORE_NAME_DEBUG)
			CCCI_ERROR_LOG(md_id, KERN,
					"[%s]-%d: snprintf fail,ret = %d\n",
