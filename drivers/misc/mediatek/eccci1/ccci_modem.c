	...
	int queue_index, struct ccci_header *msg, int is_droped)
{
	...
	if (dir == OUT) {
		...
		if (md->tx_history_ptr[queue_index] < 0 ||
			md->tx_history_ptr[queue_index] >= PACKET_HISTORY_DEPTH)
			return;
	...
	if (dir == IN) {
		if (queue_index < 0 || queue_index >= MAX_RXQ_NUM)
			return;

		if (md->rx_history_ptr[queue_index] < 0 ||
			md->rx_history_ptr[queue_index] >= PACKET_HISTORY_DEPTH)
			return;
