...
static void md_cd_traffic_monitor_handler(struct ccci_modem *md)
{
	int i;
	struct md_cd_ctrl *md_ctrl = (struct md_cd_ctrl *)md->private_data;
...
static void md_cd_traffic_monitor_func(struct timer_list *t)
{
	struct md_cd_ctrl *md_ctrl =
				from_timer(md_ctrl, t, traffic_monitor);

	md_cd_traffic_monitor_handler(md_ctrl->modem);
}
#endif
...
static void cldma_timeout_timer_func(unsigned long data)
{
	...
	ccci_md_dump_port_status(md);
	md_cd_traffic_monitor_handler(md);
...
static void md_cd_hs1_polling_timer_func(struct timer_list *t)
{
	struct md_cd_ctrl *md_ctrl =
					from_timer(md_ctrl, t, hs1_polling_timer);
	struct ccci_modem *md = md_ctrl->modem;
	//struct md_cd_ctrl *md_ctrl = (struct md_cd_ctrl *)md->private_data;
...
static int ccci_modem_probe(struct platform_device *plat_dev)
{
	...
	md_ctrl->trm_wake_lock = wakeup_source_register(NULL, md_ctrl->trm_wakelock_name);
	...
	md_ctrl->peer_wake_lock = wakeup_source_register(NULL, md_ctrl->peer_wakelock_name);
