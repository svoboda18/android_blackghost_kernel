	...
	dma_addr_t *dma_handle_out)
{
	...
	CMDQ_PROF_END(current->pid, __func__);
        ...
        CMDQ_MSG("[warn] alloc buffer pool cost %llu us\n",div_s64(alloc_cost, 1000));
	if (!va) {
		CMDQ_ERR(
...
static void cmdq_core_free_hw_buffer_pool(void *va, dma_addr_t dma_handle)
{
...
static void cmdq_core_free_reg_buffer(struct TaskStruct *task)
{
...
static void cmdq_core_alloc_reg_buffer(struct TaskStruct *task)
{
	if (task->regResults) {
...
{
	uint32_t threadNo = CMDQ_INVALID_THREAD;
	bool is_secure;
