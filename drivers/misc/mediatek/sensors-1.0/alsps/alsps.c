...
#if defined(CONFIG_NANOHUB) && defined(CONFIG_MTK_ALSPSHUB)
//add add XLWQHBLSLL-411 by tao.lan 20200428 start
#ifdef CONFIG_TRAN_STK3337_PS
    if(cxt->ps_enable)
      ps_data_report(atomic_read(&prox_state), SENSOR_STATUS_ACCURACY_HIGH);
    err = cxt->ps_ctl.enable_nodata(cxt->ps_enable);
#endif
#ifndef CONFIG_TRAN_STK3337_PS
    if(cxt->ps_enable)
     ps_data_report(atomic_read(&prox_state), SENSOR_STATUS_ACCURACY_HIGH);
    err = cxt->ps_ctl.enable_nodata(cxt->ps_enable);
#endif
//add add XLWQHBLSLL-411 by tao.lan 20200428 end
#else
