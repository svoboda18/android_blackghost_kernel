...
#include <linux/fs.h>
/* #include <linux/wakelock.h> //yangzhigang--remove  */
#include <asm/io.h>
...
static unsigned int als_setting[2] = {0};

static unsigned int als_lsc_threshold[4] = {0};
static unsigned int als_lsc_factor[4] = {0};

int fae_count = 0;

int g_tran_ps_high_thd =0;
int g_tran_ps_low_thd =0;
char g_tran_pscali_flag =0;
char g_tran_alscali_flag = 0;

int g_tran_log_count =0;
/******************************************************************************
 * configuration
******************************************************************************/
...
/*--------------------------------------------------------------------------*/
#define APS_TAG                  "[ALS/PS][STK3337] "
#define APS_FUN(f)               pr_debug(APS_TAG"%s\n", __func__)
#define APS_ERR(fmt, args...)    pr_err(APS_TAG"%s %d:"fmt, __func__, \
				__LINE__, ##args)
...
static const struct i2c_device_id stk3x3x_i2c_id[] = { {stk3x3x_DEV_NAME, 0},
							{} };
/*----------------------------------------------------------------------------*/
static int stk3x3x_i2c_probe(struct i2c_client *client,
			     const struct i2c_device_id *id);
static int stk3x3x_i2c_remove(struct i2c_client *client);
static int stk3x3x_i2c_detect(struct i2c_client *client,
...
struct stk3x3x_i2c_addr
{
	...
	/* BSP:modify XLWLHBLEJ-74 by zhigang.yang 20190611 start */
	u8  data1_als_c;		/* als c data */
	u8  data2_als_c;		/* als c data */
	/* BSP:modify XLWLHBLEJ-74 by zhigang.yang 20190611 end */
};
/*----------------------------------------------------------------------------*/
struct stk3x3x_priv
{
	...
	/*data*/
	u16         als;
	//BSP:modify XLWQHBLSLL-73 by tao.lan 20200420 start
	u16         als_data_c;
	int32_t     alsLightCali;
	atomic_t	alsFactoryCali;
	//BSP:modify XLWQHBLSLL-73 by tao.lan 20200420 end
	u16         ps;
	u8          _align;
	u16         als_level_num;
	u16         als_value_num;
...
//BSP:modify XLWLHBLEJ-74 by zhigang.yang 20190611 start
static int stk3x3x_als_light_source_dist(struct i2c_client *client,u16 als_data, int als_c);
//BSP:modify XLWLHBLEJ-74 by zhigang.yang 20190611 end
...
//yangzhigang--remove-20191115 begin
//struct wake_lock mps_lock;
//yangzhigang--remove-20191115 end
static DEFINE_MUTEX(run_cali_mutex);
//BSP: add XLWQHBLSLL-73 by tao.lan 20200422 start
static DEFINE_SPINLOCK(calibration_lock);
//BSP: add XLWQHBLSLL-73 by tao.lan 20200422 end
/*----------------------------------------------------------------------------*/
int stk3x3x_get_addr(struct stk3x3x_i2c_addr *addr)
{
	...
	//BSP:modify XLWLHBLEJ-74 by zhigang.yang 20190611 start
    addr->data1_als_c = STK_DATA1_C_REG;
	addr->data2_als_c = STK_DATA2_C_REG;
	//BSP:modify XLWLHBLEJ-74 by zhigang.yang 20190611 end
	return 0;
}
/*----------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------------*/
//BSP: add XLWQHBLSLL-73 by tao.lan 20200420 start
int stk3x3x_read_als_raw(struct i2c_client *client, u16 *data)
{
	struct stk3x3x_priv *obj = i2c_get_clientdata(client);
	int ret = 0;
	u8 buf[8] = {0};
...
//BSP: add XLWQHBLSLL-73 by tao.lan 20200420 end
/*----------------------------------------------------------------------------*/
...
//BSP:modify XLWLHBLEJ-74 by zhigang.yang 20190611 start
	...
	ret = stk3x3x_master_recv(client, obj->addr.data1_als_c, buf_c, 0x02);
	...
	als_c = (buf_c[0] << 8) | (buf_c[1]);

	obj->als_data_c = als_c;

//BSP:modify XLWLHBLEJ-1113 by zhigang.yang 20190816 start
	//APS_DBG("%s: als_c= %d,buf_c[0]=%d,buf_c[1]=%d\n",__func__,als_c,buf_c[0],buf_c[1]);
//BSP:modify XLWLHBLEJ-1113 by zhigang.yang 20190816 end
...
static int stk3x3x_als_light_source_dist(struct i2c_client *client,u16 als_data, int als_c)
{
	...
	//BSP:modify XLWLHBLEJ-655 by zhigang.yang 20190704 start
    //uint32_t als_data_u32 = als_data;
    //uint32_t als_c_u32 = als_c;
    //BSP:modify XLWLHBLEJ-655 by zhigang.yang 20190704 end
    uint16_t tran_als_ratio = 0;
	obj->alsLightCali = 0;
	if (NULL == client)
	{
		return -EINVAL;
    ...
    //BSP:modify XLWLHBLEJ-1113 by zhigang.yang 20190816 start
        //APS_ERR("[alsps][stk3337][Warning]als_c data error!\n");
    //BSP:modify XLWLHBLEJ-1113 by zhigang.yang 20190816 end

        obj->alsLightCali = als_lsc_factor[0];//CWF
    ...
    if(als_data >= 45000) {
        obj->alsLightCali = als_lsc_factor[0];
        return 0;
    }


    tran_als_ratio = als_ratio * als_ratio / als_data;
    if (tran_als_ratio > als_lsc_threshold[3]) {
        obj->alsLightCali = als_lsc_factor[3];//HZ
    } else if (tran_als_ratio > als_lsc_threshold[2]) {
        obj->alsLightCali = als_lsc_factor[2];//A
    } else if (tran_als_ratio > als_lsc_threshold[1]){
        obj->alsLightCali = als_lsc_factor[1];//D65
    } else{
        obj->alsLightCali = als_lsc_factor[0];//CWF
    }

//BSP:modify XLWLHBLEJ-655 by zhigang.yang 20190704 start
    //APS_ERR("%s: als_ratio = %d\n", __func__, als_ratio);
    //APS_ERR("%s: als_data = %d, als_c = %d \n", __func__, als_data_u32, als_c_u32);
//BSP:modify XLWLHBLEJ-655 by zhigang.yang 20190704 end
...
static int stk3x3x_enable_als(struct i2c_client *client, int enable)
{
	...
	int trc = atomic_read(&obj->trace);
//BSP:modify XLWLHBLEJ-1146 by zhigang.yang 20190926 start
    g_tran_log_count = 0;
//BSP:modify XLWLHBLEJ-1146 by zhigang.yang 20190926 end
	APS_LOG("%s: enable=%d\n", __func__, enable);
...
{
	int err;
	//BSP:modify XLWLHBLEJ-476 by zhigang.yang 20190627 start
	int tran_ps_high_thd_first =0;
	int tran_ps_low_thd_first =0;
	//BSP:modify XLWLHBLEJ-476 by zhigang.yang 20190627 end
#ifdef STK_TUNE0
#ifdef CALI_EVERY_TIME
	...
	else
	{
		...
		atomic_set(&obj->ps_low_thd_val, 0xFFFE);
	}

#endif

//BSP: add XLWLHBLEJ-476 by zhigang.yang for 20190627 start
	if(g_tran_pscali_flag == 0)
	{
		tran_ps_high_thd_first = obj->hw.ps_threshold_high;
		tran_ps_low_thd_first = obj->hw.ps_threshold_low;
	}
	else
	{
		tran_ps_high_thd_first = g_tran_ps_high_thd * 135/100;
		tran_ps_low_thd_first = g_tran_ps_low_thd * 135/100;
	}

	if ((err = stk3x3x_write_ps_high_thd(obj->client, tran_ps_high_thd_first)))
	{
		APS_ERR("write high thd error: %d\n", err);
		return err;
	}

	if ((err = stk3x3x_write_ps_low_thd(obj->client, tran_ps_low_thd_first)))
	{
	...
	APS_LOG("%s:in reg, HT=%d, LT=%d\n", __func__, tran_ps_high_thd_first, tran_ps_low_thd_first);
#else
...
//BSP: add XLWLHBLEJ-476 by zhigang.yang for 20190627 end
	return 0;
}
/*----------------------------------------------------------------------------*/
static int stk3x3x_enable_ps(struct i2c_client *client, int enable, int validate_reg)
{
	...
	int err, cur = 0, old = atomic_read(&obj->state_val);
	int trc = atomic_read(&obj->trace);
//BSP:modify XLWLHBLEJ-257 by zhigang.yang 20190617 start
	fae_count = 0;
//BSP:modify XLWLHBLEJ-257 by zhigang.yang 20190617 end
#ifdef STK_PS_DEBUG
	...
	if (enable)
	{
		cur |= (STK_STATE_EN_PS_MASK | STK_STATE_EN_INTELL_PRST_MASK);
...
#endif
#ifdef CALI_EVERY_TIME
	obj->ps_high_thd_boot = obj->crosstalk + obj->stk_ht_n_ct + STK_H_LT;
	obj->ps_low_thd_boot = obj->crosstalk + obj->stk_lt_n_ct + STK_H_LT;
...
static int stk3x3x_ps_val(void)
{
	...
	if (((ps_invalid_flag >> 5) & 0x1) || ((bgir_raw_data[0] & 0x7f) >= 50) ||
		((bgir_raw_data[1] & 0x7f) >= 50) || ((bgir_raw_data[2] & 0x7f) >= 50) || ((bgir_raw_data[3] & 0x7f) >= 50))
	{
		return -1;
	}
...
//BSP: add XLWLHBLEJ-476 by zhigang.yang for 20190627 start
	...
	obj->boot_cali = 1;
	APS_LOG("%s: ps_stat_data[0,1,2]={%d,%d,%d}",__func__,obj->ps_stat_data[0],obj->ps_stat_data[1],obj->ps_stat_data[2]);
...
static int stk_ps_tune_zero_init(struct stk3x3x_priv *obj)
{
	...
	{
		obj->ps_high_thd_boot = 0xFFFF;
		obj->ps_low_thd_boot = 0xFFFE;
	}
			...
			if (ret == 0)
			{
				...
				if (word_data > 0)
				{
						...
						&& (obj->ps_thd_update == false))
					{
						...
						if ((atomic_read(&obj->ps_high_thd_val) + STK_H_LT + 200) < (obj->ps_high_thd_boot))
						{
...
//BSP:modify XLWLHBLEJ-655 by zhigang.yang 20190704 start
				...
				APS_LOG("%s:[near eint] update HT=%d, LT=%d\n", __func__, atomic_read(&obj->ps_high_thd_val), atomic_read(&obj->ps_low_thd_val));
//BSP:modify XLWLHBLEJ-655 by zhigang.yang 20190704 end
			}
		}
		else
...
//BSP:modify XLWLHBLEJ-655 by zhigang.yang 20190704 start
					APS_LOG("%s:[far eint] Set HT=%d, LT=%d\n", __func__,
						atomic_read(&obj->ps_high_thd_val),
						atomic_read(&obj->ps_low_thd_val));
//BSP:modify XLWLHBLEJ-655 by zhigang.yang 20190704 end
				}
...
//BSP: rm XLWQHBLSLL-73 by tao.lan 20200422 start
//		enable_irq(stk3x3x_obj->irq);
//BSP: rm XLWQHBLSLL-73 by tao.lan 20200422 end
	}
	else
	{
...
static int stk3x3x_init_client(struct i2c_client *client)
{
	...
	int ps_ctrl;
	//u8 int_status;
	//BSP:add XLWLHBLEJ-11 by zhigang.yang 20190429 start
	u8 data_init;
	//BSP:add XLWLHBLEJ-11 by zhigang.yang 20190429 end
	APS_LOG("%s: In\n", __FUNCTION__);
	...
	*/
	...
	//PD
	//BSP:modify XLWLHBLEJ-11 by zhigang.yang 20190429 start
	//BSP:modify XLWLHBLEJ-74 by zhigang.yang 20190611 start
	data_init = 0x0F;//PD all Open
	//BSP:modify XLWLHBLEJ-74 by zhigang.yang 20190611 end
	stk3x3x_master_send(client, 0xA1, &data_init, 1);  //ps=ps0+ps1 
	
	//BGIR
	data_init = 0x10;
	stk3x3x_master_send(client, 0xA0, &data_init, 1);
	data_init = 0xFF;
	stk3x3x_master_send(client, 0xA8, &data_init, 1);
	data_init = 0xFF;
	stk3x3x_master_send(client, 0xA9, &data_init, 1);
	data_init = 0x64;
	stk3x3x_master_send(client, 0xAA, &data_init, 1);

	//BSP:modify XLWLHBLEJ-74 by zhigang.yang 20190611 start
	data_init = 0x04;//als AGAINx1
	//BSP:modify XLWLHBLEJ-74 by zhigang.yang 20190611 end
	stk3x3x_master_send(client, 0xDB, &data_init, 1);
	
	data_init = 0x01;
	stk3x3x_master_send(client, 0xFA, &data_init, 1);
	//BSP:modify XLWLHBLEJ-11 by zhigang.yang 20190429 end
//BSP:modify XLWLHBLEJ-791 by zhigang.yang 20190715 start
	data_init = atomic_read(&obj->alsctrl2_val);//als_c Dgain*16

	stk3x3x_master_send(client, 0x4E, &data_init, 1);
//BSP:modify XLWLHBLEJ-791 by zhigang.yang 20190715 end
...
/*----------------------------------------------------------------------------*/
static DRIVER_ATTR(als, 0644, stk3x3x_show_als, NULL);
static DRIVER_ATTR(ps, 0644, stk3x3x_show_ps, NULL);
static DRIVER_ATTR(config, 0644, stk3x3x_show_config, stk3x3x_store_config);
static DRIVER_ATTR(alslv, 0644, stk3x3x_show_alslv, stk3x3x_store_alslv);
static DRIVER_ATTR(alsval, 0644, stk3x3x_show_alsval, stk3x3x_store_alsval);
static DRIVER_ATTR(trace, 0644, stk3x3x_show_trace, stk3x3x_store_trace);
static DRIVER_ATTR(status, 0644, stk3x3x_show_status, NULL);
static DRIVER_ATTR(send, 0644, stk3x3x_show_send, stk3x3x_store_send);
static DRIVER_ATTR(recv, 0644, stk3x3x_show_recv, stk3x3x_store_recv);
static DRIVER_ATTR(reg, 0644, stk3x3x_show_reg, NULL);
static DRIVER_ATTR(allreg, 0644, stk3x3x_show_allreg, NULL);
static DRIVER_ATTR(pscalibration, 0644, stk3x3x_show_ps_offset, stk3x3x_set_ps_offset);
static DRIVER_ATTR(pthredcalibration, 0644, stk3x3x_show_pthreshold_calibration, NULL);
#ifdef STK_TUNE0
static DRIVER_ATTR(cali, 0644, stk3x3x_show_cali, NULL);
static DRIVER_ATTR(maxdiff, 0644, stk3x3x_ps_maxdiff_show, stk3x3x_ps_maxdiff_store);
static DRIVER_ATTR(ltnct, 0644, stk3x3x_ps_ltnct_show, stk3x3x_ps_ltnct_store);
static DRIVER_ATTR(htnct, 0644, stk3x3x_ps_htnct_show, stk3x3x_ps_htnct_store);
#endif
static DRIVER_ATTR(ps_offset, 0644, NULL, stk3x3x_store_ps_offset);
/*----------------------------------------------------------------------------*/
...
static int stk3x3x_get_als_value(struct stk3x3x_priv *obj, u16 als)
{
	int idx;
	int invalid = 0;
	//BSP:modify XLWLHBLEJ-36 by zhigang.yang 20190523 start
	unsigned int lum = 0;
	//BSP:modify XLWLHBLEJ-36 by zhigang.yang 20190523 end
	...
	if (!invalid)
	{
		...
		//BSP:modify XLWLHBLEJ-36 by zhigang.yang 20190523 start
		lum = (obj->hw.als_value[idx] -obj->hw.als_value[idx-1]) * (als - obj->hw.als_level[idx-1])/ (obj->hw.als_level[idx] - obj->hw.als_level[idx-1]);
		lum += obj->hw.als_value[idx-1];
...
static long stk3x3x_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case ALSPS_GET_ALS_RAW_DATA:
		if ((err = stk3x3x_read_als_raw(obj->client, &obj->als)))
		{
			goto err_out;
		}
...
//BSP:modify XLWLHBLEJ-31 by zhigang.yang 20190522 start

			ps_result = 1;
		else
			ps_result = 0;
//BSP:modify XLWLHBLEJ-31 by zhigang.yang 20190522 end
...
//BSP:modify XLWLHBLEJ-476 by zhigang.yang 20190627 start
		APS_ERR("%s set threshold high: %d, low: %d ,ps-normal=%d\n", __func__, threshold[0], threshold[1],obj->ps_cali);
//BSP:modify XLWLHBLEJ-476 by zhigang.yang 20190627 end
...
//BSP:modify XLWLHBLEJ-257 by zhigang.yang 20190617 start
        if(threshold[0] > threshold[1]){
			obj->stk_ht_n_ct = threshold[0];
			obj->stk_lt_n_ct = threshold[1];
        }
//BSP:modify XLWLHBLEJ-257 by zhigang.yang 20190617 end
//BSP:modify XLWLHBLEJ-476 by zhigang.yang 20190627 start
		    g_tran_ps_high_thd = threshold[0] + obj->ps_cali;
		    g_tran_ps_low_thd = threshold[1] + obj->ps_cali;
			g_tran_pscali_flag = 1;
//BSP:modify XLWLHBLEJ-476 by zhigang.yang 20190627 end
		if ((err = stk3x3x_write_ps_high_thd(obj->client, atomic_read(&obj->ps_high_thd_val))))
		{
    ...
    case ALSPS_ALS_SET_CALI:
		if (copy_from_user(&cali, ptr, sizeof(cali)))
		{
			err = -EFAULT;
			goto err_out;
		}
		APS_ERR("%s set alslightcali: %d.\n", __func__,cali);
		atomic_set(&obj->alsFactoryCali, cali);
        g_tran_alscali_flag = 1;
...
//BSP: modify XLWQHBLSLL-73 by tao.lan 20200420 start
		als_raw = obj->als;
        if (g_tran_alscali_flag == 0) {
            if(!stk3x3x_als_light_source_dist(obj->client,obj->als,obj->als_data_c)){
                obj->als =obj->als * 1000 /obj->alsLightCali;
            }

            if(obj->alsLightCali == als_lsc_factor[0]) {
                obj->als =obj->als * 1000 /obj->alsLightCali;
		...
		if (*value < 0)
            ...
            *value = (als_raw * atomic_read(&obj->alsFactoryCali)) / 1000;
        }
        if (g_tran_log_count == 10){
            g_tran_log_count = 0;
            APS_DBG("[alsps][stk3337][debug]%s: als_raw = %d, als_reportData =%d ,alsLightCali=%d\n",__func__,als_raw,*value,obj->alsLightCali);
        }else{
            g_tran_log_count++;
        }
//BSP: modify XLWQHBLSLL-73 by tao.lan 20200420 end
...
static int stk3x3x_init_reg(struct i2c_client *client)
{
	...
	//BSP:modify XLWLHBLEJ-64 by zhigang.yang 20190604 start
	//uint8_t i2c_config_reg = 0;
	//BSP:modify XLWLHBLEJ-64 by zhigang.yang 20190604 end

	obj = stk3x3x_obj;
	printk("client->addr======%x\n", client->addr);
	client->addr = 0x47;  //0x47
	printk("client->addr======%x\n", client->addr);
	obj->client = client;
	i2c_set_clientdata(client, obj);
	atomic_set(&obj->als_debounce, 110);//reference IT
	atomic_set(&obj->als_deb_on, 0);
...
//BSP:modify XLWQHBLSLL-73 by tao.lan 20200423 start
    atomic_set(&obj->alsFactoryCali, 0);
//BSP:modify XLWQHBLSLL-73 by tao.lan 20200423 end

//BSP:modify XLWLHBLEJ-64 by zhigang.yang 20190604 start
	//i2c_config_reg = STK_PS_PRS4 | STK_PS_GAIN8 | STK_PS_IT100;
	//atomic_set(&obj->psctrl_val, i2c_config_reg);
	atomic_set(&obj->psctrl_val, ps_setting[0]);
	...
	atomic_set(&obj->alsctrl2_val, als_setting[1]);

	obj->ledctrl_val = ps_setting[4];
//BSP:modify XLWLHBLEJ-64 by zhigang.yang 20190604 end

	obj->wait_val = STK_WAIT100;
...
/*----------------------------------------------------------------------------*/
//BSP:modify XLWQHBLSLL-14 by tao.lan 20200414 start
#ifdef CONFIG_TRAN_SYSTEM_DEVINFO
extern void app_get_als_sensor_name(char *als_name);
extern void app_get_ps_sensor_name(char *ps_name);
#endif
//BSP:modify XLWQHBLSLL-14 by tao.lan 20200414 end
//BSP:modify XLWQHBLSLL-73 by tao.lan 20200422 start
static int als_set_cali(uint8_t *data, uint8_t count)
{
	int32_t *buf = (int32_t *)data;
    struct stk3x3x_priv *obj = stk3x3x_obj;
	spin_lock(&calibration_lock);
	atomic_set(&obj->alsFactoryCali, buf[0]);
	spin_unlock(&calibration_lock);

    APS_LOG("als_set_cali : alsFactoryCali = %d\n", atomic_read(&obj->alsFactoryCali));

    if(atomic_read(&obj->alsFactoryCali) != 0)
    {
        g_tran_alscali_flag = 1;
    } else {
        g_tran_alscali_flag = 0;
    }
	return 0;
}
//BSP:modify XLWQHBLSLL-73 by tao.lan 20200422 end
...
//BSP: modify XLWLHBLEJ-64 by zhigang.yang 20190604 start
	if (client->dev.of_node) {
		...
		if (err)
			return -EFAULT;
		APS_LOG("[alsps][stk3337][info] %s,get dts info success,ps_setting[]=[0x%x,%d,%d,%d,0x%x],als_setting[]=[0x%x,0x%x]\n",__func__,
		ps_setting[0],ps_setting[1],ps_setting[2],ps_setting[3],ps_setting[4],als_setting[0],als_setting[1]);

		//BSP:modify XLWLHBLEJ-74 by zhigang.yang 20190611 start
		err = of_property_read_u32_array(client->dev.of_node, "als_lsc_threshold", als_lsc_threshold, ARRAY_SIZE(als_lsc_threshold));
		if (err){
			APS_ERR("[alsps][stk3337][info] %s, get als_lsc_threshold info fail",__func__);
			return -EFAULT;
		}
		err = of_property_read_u32_array(client->dev.of_node, "als_lsc_factor", als_lsc_factor, ARRAY_SIZE(als_lsc_factor));
		if (err){
			APS_ERR("[alsps][stk3337][info] %s, get als_lsc_threshold info fail",__func__);
			return -EFAULT;
		}
		APS_LOG("[alsps][stk3337][info] %s,get dts info2 success,als_lsc_threshold[]={%d,%d,%d,%d},als_lsc_factor[]={%d,%d,%d,%d}\n",__func__,
		als_lsc_threshold[0],als_lsc_threshold[1],als_lsc_threshold[2],als_lsc_threshold[3],als_lsc_factor[0],als_lsc_factor[1],als_lsc_factor[2],als_lsc_factor[3]);
		//BSP:modify XLWLHBLEJ-74 by zhigang.yang 20190611 end
	}
//BSP: modify XLWLHBLEJ-64 by zhigang.yang 20190604 end
...
//BSP:modify XLWQHBLSLL-14 by tao.lan 20200414 start
	#ifdef CONFIG_TRAN_SYSTEM_DEVINFO
		app_get_als_sensor_name("stk3337_L");
        app_get_ps_sensor_name("stk3337_P");
	#endif
//BSP:modify XLWQHBLSLL-14 by tao.lan 20200414 end
...
//yangzhigang--modify-20191115 begin
	misc_deregister(&stk3x3x_device);
//yangzhigang--modify-20191115 end
