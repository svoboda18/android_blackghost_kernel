...
static char mxc6655_prox_version[64]="MXC6655-Q-MEMSIC";
static char mxc6655_prox_vendor_name[64]="MXC6655-MEMSIC";
    DEV_ATTR_DECLARE(gsensor)
    DEV_ATTR_DEFINE("version",mxc6655_prox_version)
    DEV_ATTR_DEFINE("vendor",mxc6655_prox_vendor_name)
...
static int MXC6655_SaveData(int buf[MXC6655_AXES_NUM])
{

	mutex_lock(&mxc6655_mutex);
	cali_sensor_data = buf[2];

	mutex_unlock(&mxc6655_mutex);

    return 0;
}

static int MXC6655_ReadTemp(struct i2c_client *client, s8 *data)
{
	u8 addr = MXC6655_REG_TEMP;
	int err = 0;
	s8 temp = 0;
	...
	if((err = mxc6655_i2c_read_block(client, addr, &temp, 1)))
	{
		GSE_ERR("error: %d\n", err);
	}
	*data = temp;
...
static int MXC6655_ReadDant(struct i2c_client *client, int dant[MXC6655_AXES_NUM+1])
{
	u8 addr = MXC6655_REG_X;
	u8 buf[MXC6655_DATA_LEN+1] = {0};
	int err = 0;
	...
	if((err = mxc6655_i2c_read_block(client, addr, buf, MXC6655_DATA_LEN+1)))
	{
		GSE_ERR("error: %d\n", err);
	}
	else
		...
		dant[MXC6655_AXIS_X] = (s16)((s16)buf[0] << 8 | (s16)buf[1]) >> 4;
		dant[MXC6655_AXIS_Y] = (s16)((s16)buf[2] << 8 | (s16)buf[3]) >> 4;
		dant[MXC6655_AXIS_Z] = (s16)((s16)buf[4] << 8 | (s16)buf[5]) >> 4;
		dant[MXC6655_AXIS_Z+1] = (s16)buf[6];
...
#ifdef CONFIG_COMPAT
static long mxc6655_compat_ioctl(struct file *file, unsigned int cmd,
       unsigned long arg)
{
    long err = 0;
...
static long mxc6655_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		...
		case GSENSOR_IOCTL_GET_DATA:
			...
			err = MXC6655_ReadSensorData(client, vec,MXC6655_AXES_NUM);
		...
		case GSENSOR_IOCTL_GET_TEMP:
			...
			err = MXC6655_ReadTemp(client, &temp);
		...
		case GSENSOR_IOCTL_GET_DANT:
			...
			err = MXC6655_ReadDant(client, dant);
		...
		case GSENSOR_IOCTL_READ_REG:
			...
			if((err = mxc6655_i2c_read_block(client, test, &buf, 1)))
			{
				GSE_ERR("error: %d\n", err);
			}
...
#ifdef CONFIG_COMPAT
		.compat_ioctl = mxc6655_compat_ioctl,
#endif
};
...
static int mxc6655_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk("yxch client->addr= %x\r\n",client->addr);
	printk("yxch direction= %d\r\n",obj->hw->direction);
	//client->addr=0x15;
