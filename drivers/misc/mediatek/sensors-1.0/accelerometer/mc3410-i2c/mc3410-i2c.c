			       ...
			       int bufsize)
{
	...
	return sprintf(buf, "MC3XXX Chip");
}
				 ...
				 int nBufSize)
{
	...
	return sprintf(pbBuf, "%04x %04x %04x", _naAccelData[MC3XXX_AXIS_X],
		       _naAccelData[MC3XXX_AXIS_Y],
		       _naAccelData[MC3XXX_AXIS_Z]);
}
				     ...
				     char *pbBuf)
{
	...
	return _tLength < PAGE_SIZE ? _tLength : -EINVAL;;
}
					...
					char *pbBuf)
{
	...
	res = snprintf(pbBuf, PAGE_SIZE, "%d\n", _bChipValidation);
	return res < PAGE_SIZE ? res : -EINVAL;
}
