...
static int stk8baxx_store_in_file(u8 offset[], u8 status, u32 variance[])
{
	...
	err = stk8baxx_write_file(1, w_buf);
	//err = stk8baxx_write_file(0, w_buf); //wangzhongping changed for solving kernel reboot.
	//if(err == 0)
...
static ssize_t show_layout_value(struct device_driver *ddri, char *buf)
{
	...
	return sprintf(buf, "CUST:%d\n",atomic_read(&obj->layout));
}
/*----------------------------------------------------------------------------*/
...
static DRIVER_ATTR(status, 0644, show_status_value, NULL);
static DRIVER_ATTR(powerstatus, 0644, show_power_status_value, NULL);
static DRIVER_ATTR(allreg, 0644, show_allreg_value, NULL);
static DRIVER_ATTR(send,    0644, show_send_value,  store_send_value);
static DRIVER_ATTR(recv,    0644, show_recv_value,  store_recv_value);
...
static int stk8baxx_batch(int flag, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs)
{
	...
	/*Fix Me*/
	GSE_LOG("stk8baxx set delay = (%d) OK!\n", value);
	return 0;
}
...
static int stk8baxx_ReadRawData(struct i2c_client *client, char *buf)
{
	...
	if(false == obj_i2c_data->sensor_power) {
		res = STK8BAXX_SetPowerMode(client, true);
		if (res) {
			GSE_ERR("Power on stk8baxx error %d!\n", res);
			return -4;
	...
	res = STK8BAXX_ReadData(client, sensor_data);
	if (res) {
		GSE_ERR("I2C error: ret value=%d", res);
		return -EIO;
	}
	sprintf(buf, "%04x %04x %04x", sensor_data[STK8BAXX_AXIS_X],
	sensor_data[STK8BAXX_AXIS_Y], sensor_data[STK8BAXX_AXIS_Z]);

	return 0;
}


static int stk8baxx_factory_enable_sensor(bool enabledisable, int64_t sample_periods_ms)
{
	int err;
	...
	err = stk8baxx_batch(0, sample_periods_ms * 1000000, 0);
	if (err) {
...
static int stk8baxx_factory_get_data(int32_t data[3], int *status)
{
	return gsensor_get_data(&data[0], &data[1], &data[2], status);

}
static int stk8baxx_factory_get_raw_data(int32_t data[3])
{
/*	char strbuf[STK8BAXX_BUFSIZE] = { 0 };
	stk8baxx_ReadRawData(stk8baxx_i2c_client, strbuf);
	GSE_LOG("support stk8baxx_factory_get_raw_data!\n");
*/	
	char strbuf[STK8BAXX_BUFSIZE] = { 0 };
	stk8baxx_ReadRawData(stk8baxx_i2c_client, strbuf);
...
static int stk8baxx_factory_enable_calibration(void)
{
	return 0;
}
static int stk8baxx_factory_clear_cali(void)
{
/*	int err = 0;
	err = STK8BAXX_ResetCalibration(stk8baxx_i2c_client);
	if (err) {
		GSE_ERR("stk8baxx_ResetCalibration failed!\n");
		return -1;
	}*/
	int err = 0;
	err = STK8BAXX_ResetCalibration(stk8baxx_i2c_client);
...
static int stk8baxx_factory_set_cali(int32_t data[3])
{
/*	int err = 0;
	int cali[3] = { 0 };
	obj_i2c_data->cali_sw[STK8BAXX_AXIS_X] += data[0];
	obj_i2c_data->cali_sw[STK8BAXX_AXIS_Y] += data[1];
	obj_i2c_data->cali_sw[STK8BAXX_AXIS_Z] += data[2];

	cali[STK8BAXX_AXIS_X] = data[0] * gsensor_gain.x / GRAVITY_EARTH_1000;
	cali[STK8BAXX_AXIS_Y] = data[1] * gsensor_gain.y / GRAVITY_EARTH_1000;
	cali[STK8BAXX_AXIS_Z] = data[2] * gsensor_gain.z / GRAVITY_EARTH_1000;
	err = STK8BAXX_WriteCalibration(stk8baxx_i2c_client, cali);
	if (err) {
		GSE_ERR("stk8baxx_WriteCalibration failed!\n");
		return -1;
	}*/
	int err = 0;
	int cali[3] = { 0 };

	cali[STK8BAXX_AXIS_X] = data[0] * obj_i2c_data->reso->sensitivity / GRAVITY_EARTH_1000;
	cali[STK8BAXX_AXIS_Y] = data[1] * obj_i2c_data->reso->sensitivity / GRAVITY_EARTH_1000;
	cali[STK8BAXX_AXIS_Z] = data[2] * obj_i2c_data->reso->sensitivity / GRAVITY_EARTH_1000;
	err = STK8BAXX_WriteCalibration(stk8baxx_i2c_client, cali);
	if (err) {
		GSE_ERR("stk8baxx_WriteCalibration failed!\n");
		return -1;
...
static int stk8baxx_factory_get_cali(int32_t data[3])
{
	/*data[0] = obj_i2c_data->cali_sw[STK8BAXX_AXIS_X];
	data[1] = obj_i2c_data->cali_sw[STK8BAXX_AXIS_Y];
	data[2] = obj_i2c_data->cali_sw[STK8BAXX_AXIS_Z];*/
	int err = 0;
	int cali[3] = { 0 };

	err = STK8BAXX_ReadCalibration(stk8baxx_i2c_client, cali);
	if (err) {
		GSE_ERR("stk8baxx_ReadCalibration failed!\n");
		return -1;
	}
	data[0] = cali[STK8BAXX_AXIS_X] * GRAVITY_EARTH_1000 / obj_i2c_data->reso->sensitivity;
	data[1] = cali[STK8BAXX_AXIS_Y] * GRAVITY_EARTH_1000 / obj_i2c_data->reso->sensitivity;
	data[2] = cali[STK8BAXX_AXIS_Z] * GRAVITY_EARTH_1000 / obj_i2c_data->reso->sensitivity;	
	return 0;
}
static int stk8baxx_factory_do_self_test(void)
{
	return 0;
}

static struct accel_factory_fops stk8baxx_factory_fops = {
	.enable_sensor = stk8baxx_factory_enable_sensor,
	.get_data = stk8baxx_factory_get_data,
	.get_raw_data = stk8baxx_factory_get_raw_data,
	.enable_calibration = stk8baxx_factory_enable_calibration,
	.clear_cali = stk8baxx_factory_clear_cali,
	.set_cali = stk8baxx_factory_set_cali,
	.get_cali = stk8baxx_factory_get_cali,
	.do_self_test = stk8baxx_factory_do_self_test,
};

static struct accel_factory_public stk8baxx_factory_device = {
	.gain = 1,
	.sensitivity = 1,
	.fops = &stk8baxx_factory_fops,
};
...
static int stk8baxx_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	/* factory */
	err = accel_factory_device_register(&stk8baxx_factory_device);
	if (err) {
...
static int stk8baxx_i2c_remove(struct i2c_client *client)
{
	...
	accel_factory_device_deregister(&stk8baxx_factory_device);
