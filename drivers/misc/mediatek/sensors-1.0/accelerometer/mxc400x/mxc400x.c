...
static int MXC400X_ReadDant(struct i2c_client *client, int dant[MXC400X_AXES_NUM])
{
...
static int MXC400X_ReadSensorDataFactory(struct i2c_client *client, char *buf, int bufsize)
{
	...
	else
	{
		...
		sprintf(buf, "factory_read_mxc400x: acc: x:%04x y:%04x z:%04x",acc[MXC400X_AXIS_X],acc[MXC400X_AXIS_Y],cali_sensor_data);
