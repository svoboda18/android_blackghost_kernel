...
#endif
#ifdef CONFIG_PM_SLEEP //add weifeng.niu for kernel 4.4 API
...
static const struct of_device_id accel_of_match[] = {
	{.compatible = "mediatek,gsensor_mxc4005x"},
	{},
};
#endif
...
#ifdef CONFIG_PM_SLEEP //add weifeng.niu for kernel 4.4 API
//static int mxc400x_suspend(struct i2c_client *client, pm_message_t msg)  //delete weifeng.niu  for kernel 4.4
static int mxc400x_suspend(struct device *dev)
{
struct i2c_client *client = to_i2c_client(dev); //add weifeng.niu for kernel 4.4
	
	 struct mxc400x_i2c_data *obj = i2c_get_clientdata(client);
	 int err = 0;
	 GSE_INFO("mxc400x_suspend !\n ");
//	 if(msg.event == PM_EVENT_SUSPEND)   //delete weifeng.niu  for kernel 4.4
//	 {   //delete weifeng.niu  for kernel 4.4
		 if(obj == NULL)
		 {
			 GSE_ERR("null mxc400x!!\n");
			 return -EINVAL;
...
//static int mxc400x_resume(struct i2c_client *client)  //delete weifeng.niu  for kernel 4.4
static int mxc400x_resume(struct device *dev)
{
struct i2c_client *client = to_i2c_client(dev);   //add weifeng.niu for kernel 4.4
...
/*     //delete weifeng.niu for kernel 4.4 API
...
#ifdef CONFIG_TRAN_SYSTEM_DEVINFO
        app_get_g_sensor_name("mxc400x-new_3axis");
#endif
	mxc400x_init_flag = 0;
	GSE_INFO("%s: OK\n", __func__);
	return 0;
