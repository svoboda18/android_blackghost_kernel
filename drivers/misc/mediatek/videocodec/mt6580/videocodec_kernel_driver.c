				...
				unsigned long arg)
{
	...
	case VCODEC_INITHWLOCK:
	{
		...
		for (i = 0; i < hwoal_reg.u4NumOfRegister; i++) {
			...
			spin_lock_irqsave(&OalHWContextLock, ulFlags);
			pr_info("[VCODEC][REG_INFO_2][ioremap] [%d] pa = 0x%lx  kva = 0x%lx, u4NumOfRegister %d\n",
					i,addr_pa, kva, hwoal_reg.u4NumOfRegister);
			/* oal_mem_status[i].u4ReadAddr; */
	...
	if (Driver_Open_Count == 0) {
		...
		for (i = 0; i < VCODEC_INST_NUM; i++) {
			...
			for (j = 0; j < OALMEM_STATUS_NUM; j++) {
			    if(hw_ctx[i].oalmem_status[j].u4ReadAddr != 0)
			    {
			        pr_info("[VCODEC][vcodec_release][iounmap] [%d][%d] kva = 0x%lx, u4NumOfRegister %d\n",
					    i,j, hw_ctx[i].oalmem_status[j].u4ReadAddr, hw_ctx[i].u4NumOfRegister);
			        iounmap((void*)hw_ctx[i].oalmem_status[j].u4ReadAddr);
			    }
...
MODULE_DESCRIPTION("MT6580 Vcodec Driver");
MODULE_LICENSE("GPL");
