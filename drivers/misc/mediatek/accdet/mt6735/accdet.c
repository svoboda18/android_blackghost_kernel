...
static int cat_register(char *buf)
{
	...
	for (i = ACCDET_RSV; i <= ACCDET_RSV_CON1; i += 2) {
		ret = sprintf(buf_temp, "ACCDET_ADDR[0x%x]=0x%x\n", (ACCDET_BASE + i), pmic_pwrap_read(ACCDET_BASE + i));
        if (ret < 0)
          ACCDET_DEBUG("[Accdet]sprintf failed\n");
		strncat(buf, buf_temp, strlen(buf_temp));
	}

	ret = sprintf(buf_temp, "TOP_RST_ACCDET[0x%x]=0x%x\n", TOP_RST_ACCDET, pmic_pwrap_read(TOP_RST_ACCDET));
	if (ret < 0)
      ACCDET_DEBUG("[Accdet]sprintf failed\n");
	strncat(buf, buf_temp, strlen(buf_temp));
	ret = sprintf(buf_temp, "INT_CON_ACCDET[0x%x]=0x%x\n", INT_CON_ACCDET, pmic_pwrap_read(INT_CON_ACCDET));
    if (ret < 0)
      ACCDET_DEBUG("[Accdet]sprintf failed\n");
	strncat(buf, buf_temp, strlen(buf_temp));
	ret = sprintf(buf_temp, "TOP_CKPDN[0x%x]=0x%x\n", TOP_CKPDN, pmic_pwrap_read(TOP_CKPDN));
    if (ret < 0)
      ACCDET_DEBUG("[Accdet]sprintf failed\n");
	strncat(buf, buf_temp, strlen(buf_temp));
	ret = sprintf(buf_temp, "ACCDET_ADC_REG[0x%x]=0x%x\n", ACCDET_ADC_REG, pmic_pwrap_read(ACCDET_ADC_REG));
    if (ret < 0)
      ACCDET_DEBUG("[Accdet]sprintf failed\n");
	strncat(buf, buf_temp, strlen(buf_temp));
...
static ssize_t show_accdet_state(struct device_driver *ddri, char *buf)
{
	...
	if (ret < 0)
          ACCDET_DEBUG("[Accdet]snprintf failed\n");
...
int mt_accdet_probe(struct platform_device *dev)
{
	...
	// wake lock
	//------------------------------------------------------------------*/
	accdet_suspend_lock = wakeup_source_register(NULL, "accdet_suspend_lock");
	accdet_irq_lock = wakeup_source_register(NULL, "accdet_irq_lock");
	accdet_key_lock = wakeup_source_register(NULL, "accdet_key_lock");
