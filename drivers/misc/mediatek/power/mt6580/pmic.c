...
#if defined(CONFIG_MTK_GAUGE_VERSION)
...
static struct wakeup_source *dlpt_notify_lock;
...
#if defined(CONFIG_MT6580_USE_FCHR_ENB_KEY)
...
static struct wakeup_source *pmicThread_lock;
...
void PMIC_INIT_SETTING_V1(void)
{
	...
	ret = pmic_config_interface(0xC, 0x3, 0x7, 1);	/*VBAT_OV_VTH, 4.45V, */
#else
	ret = pmic_config_interface(0xC, 0x1, 0x7, 1);  /*VBAT_OV_VTH, 4.3V, */
#endif
...
static int __init pmic_mt_init(void)
{
	...
	pmicThread_lock = wakeup_source_register(NULL, "pmicThread_lock");
#ifdef BATTERY_PERCENT_PROTECT
	bat_percent_notify_lock =
		wakeup_source_register(NULL, "bat_percent_notify_lock");
#endif	/* #ifdef BATTERY_PERCENT_PROTECT */

#ifdef DLPT_FEATURE_SUPPORT
	dlpt_notify_lock = wakeup_source_register(NULL, "dlpt_notify_lock");
#endif	/* #ifdef DLPT_FEATURE_SUPPORT */
