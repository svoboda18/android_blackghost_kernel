...
unsigned char *pinputdata)
{
	...
	/* fix warning MSG   CAM_CALDB("[CAM_CAL] iReadData finial address is
	 %d length is %d buffer address is 0x%x\n",u4CurrentOffset, i4ResidueDataLength, pBuff); */
...
*/
	...
	/* fix warning MSG     CAM_CALDB("[CAM_CAL] init Working buffer address
	 0x%x  command is 0x%08x\n", pWorkingBuff, a_u4Command); */
	...
	case CAM_CALIOC_G_READ:
		...
		/* fix warning MSG            CAM_CALDB("[CAM_CAL] Before
		 read Working buffer address 0x%x\n", pWorkingBuff); */

		i4RetValue = iReadData((u16)ptempbuf->u4Offset, ptempbuf->u4Length, pWorkingBuff);
		/* fix warning MSG            CAM_CALDB("[CAM_CAL] After read Working
		 buffer address 0x%x\n", pWorkingBuff); */
	...
	if (_IOC_READ & _IOC_DIR(a_u4Command)) {
		...
		/* fix warning MSG        CAM_CALDB("[CAM_CAL] to user  Working buffer
		address 0x%x\n", pWorkingBuff); */
...
static inline int RegisterCAM_CALCharDrv(void)
{
	...
	CAM_CAL_device = device_create(CAM_CAL_class, NULL, g_CAM_CALdevno, NULL,
	CAM_CAL_DRVNAME);
