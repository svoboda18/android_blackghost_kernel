	...
	unsigned int end, unsigned int port_mask, unsigned int larb_mask)
{
	...
	M4ULOG_HIGH("%s [%d], start=0x%x, end=0x%x, write: %d, port_mask=0x%x, larb_mask=0x%x\n",
...
int m4u_insert_seq_range(int port, unsigned int MVAStart, unsigned int MVAEnd)
{
	...
	M4ULOG_MID("%s: module:%s, MVAStart:0x%x, MVAEnd:0x%x\n",
...
static void larb_backup(struct larb_monitor *h, int larb_idx)
{
	...
	if (larb_idx >= SMI_LARB_NR || larb_idx < 0) {
...
irqreturn_t MTK_M4U_isr(int irq, void *dev_id)
{
...

	{
		...
		if (IntrSrc & (F_INT_MMU0_MAIN_MSK | F_INT_MMU0_MAU_MSK))
			...
			M4UMSG("m4u interrupt err: status = 0x%x\n", IntrSrc);
		...
		if (IntrSrc & F_INT_TRANSLATION_FAULT(m4u_slave_id)) {
			...
			if (gM4uPort[m4u_port].enable_tf == 1 &&
				...
				if (m4u_port < M4U_PORT_UNKNOWN &&
				    gM4uPort[m4u_port].fault_data == NULL) {
					m4u_aee_print(
		...
		for (i = 0; i < SMI_LARB_NR; i++) {
			...
			if (node == NULL)
				...
				M4U_WriteReg32(gLarbBaseAddr[i],
					       SMI_LARB_DOMN_0, 0x33333333);
				M4U_WriteReg32(gLarbBaseAddr[i],
					       SMI_LARB_DOMN_1, 0x33333333);
				M4U_WriteReg32(gLarbBaseAddr[i],
					       SMI_LARB_DOMN_2, 0x33333333);
				M4U_WriteReg32(gLarbBaseAddr[i],
					       SMI_LARB_DOMN_3, 0x33333333);
			}
		}
	}
/* ============================================= */
...

	{
		...
		if (!gM4U_4G_DRAM_Mode)
			...
			M4U_WriteReg32(gM4UBaseAddr[m4u_id],
				REG_MMU_IVRP_PADDR,
				(unsigned int)
				F_MMU_IVRP_4G_DRAM_PA_SET(ProtectPA));
...
int m4u_hw_init(struct m4u_device *m4u_dev, int m4u_id)
{
	...
	if (request_irq(m4u_dev->irq_num[m4u_id],
			MTK_M4U_isr, IRQF_TRIGGER_LOW, "m4u", NULL)) {
