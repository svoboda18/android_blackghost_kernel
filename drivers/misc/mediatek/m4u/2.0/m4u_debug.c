...
enum m4u_callback_ret m4u_test_callback(int alloc_port, unsigned int mva,
...
int m4u_test_ddp(unsigned int prot)
{
	...
	struct m4u_client *client = m4u_create_client();
...
test_fault_callback(int port, unsigned int mva, void *data)
{
	if (data != NULL)
