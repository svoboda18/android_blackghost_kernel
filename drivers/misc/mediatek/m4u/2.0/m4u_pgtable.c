			...
			unsigned int pgprot)
{
	...
	if (likely(!imu_pgd_val(*pgd))) {
		...
		M4ULOG_LOW(
		    "%s: pgd: 0x%lx, pte_va:0x%lx, pte_pa: 0x%lx, val: 0x%x\n",
			...
			unsigned int prot)
{
	...
	M4ULOG_LOW("%s: mva:0x%x,pgd:0x%lx(0x%lx+0x%x), pa:0x%pa,val:0x%x\n",
		...
		phys_addr_t pa, unsigned int prot)
{
	...
	M4ULOG_LOW(
	    "%s: mva: 0x%x, pte: 0x%p (0x%lx + 0x%x), pa: 0x%pa, val: 0x%x\n",
		       ...
		       phys_addr_t paddr, unsigned int size, unsigned int prot)
{
	...
	if (!IS_ALIGNED(iova | paddr | size, min_pagesz)) {
		M4UMSG("unaligned: iova 0x%x pa 0x%pa sz 0x%x min_pgsz 0x%x\n",
