...
enum DPI_STATUS ddp_dpi_ConfigPclk(cmdqRecHandle cmdq, unsigned int clk_req, enum LCM_POLARITY polarity)
{
	unsigned clksrc = 0;
	...
	ctrl.CLK_POL = (LCM_POLARITY_FALLING == polarity) ? 1 : 0;
	DPI_OUTREGBIT(cmdq, struct DPI_REG_OUTPUT_SETTING, DPI_REG->OUTPUT_SETTING, CLK_POL, ctrl.CLK_POL);

	return DPI_STATUS_OK;
}

enum DPI_STATUS ddp_dpi_ConfigDE(cmdqRecHandle cmdq, enum LCM_POLARITY polarity)
{
	struct DPI_REG_OUTPUT_SETTING pol = DPI_REG->OUTPUT_SETTING;

	pol.DE_POL = (LCM_POLARITY_FALLING == polarity) ? 1 : 0;
	DPI_OUTREGBIT(cmdq, struct DPI_REG_OUTPUT_SETTING, DPI_REG->OUTPUT_SETTING, DE_POL, pol.DE_POL);

	return DPI_STATUS_OK;
}

enum DPI_STATUS ddp_dpi_ConfigVsync(cmdqRecHandle cmdq, enum LCM_POLARITY polarity, unsigned pulseWidth,
			       unsigned backPorch, unsigned frontPorch)
{
	...
	pol.VSYNC_POL = (LCM_POLARITY_FALLING == polarity) ? 1 : 0;
...
enum DPI_STATUS ddp_dpi_ConfigHsync(cmdqRecHandle cmdq, enum LCM_POLARITY polarity, unsigned pulseWidth,
			       unsigned backPorch, unsigned frontPorch)
{
	...
	pol.HSYNC_POL = (LCM_POLARITY_FALLING == polarity) ? 1 : 0;
