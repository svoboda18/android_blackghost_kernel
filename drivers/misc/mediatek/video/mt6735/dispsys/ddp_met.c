	...
	for (i = 0; i < layerCnt; i++) {
		...
		if (ovlInfo[i].layer_en) {
			...
			if (sLayerBufAddr[index][idx] != ovlInfo[i].addr) {
				...
				if (met_tag_oneshot_symbol)
					met_tag_oneshot_symbol(DDP_IRQ_LAYER_FPS_ID, tag_name, i + 1);
			}
			if (sLayerBufFmt[index][idx] != ovlInfo[i].fmt) {
				...
				if (met_tag_oneshot_symbol)
					met_tag_oneshot_symbol(DDP_IRQ_LAYER_FORMAT_ID, tag_name, i + 1);
			}
			if (sLayerBufWidth[index][idx] != ovlInfo[i].src_w) {
				...
				if (met_tag_oneshot_symbol)
					met_tag_oneshot_symbol(DDP_IRQ_LAYER_SIZE_ID, tag_name, ovlInfo[i].src_w);
			}
			if (sLayerBufHeight[index][idx] != ovlInfo[i].src_h) {
				...
				if (met_tag_oneshot_symbol)
					met_tag_oneshot_symbol(DDP_IRQ_LAYER_SIZE_ID, tag_name, ovlInfo[i].src_h);
			}
		...
		if ((i == (OVL_LAYER_NUM_PER_OVL - 1)) || (i == (OVL_LAYER_NUM - 1))) {
			if (enLayerCnt) {
				...
				if (met_tag_oneshot_symbol)
					met_tag_oneshot_symbol(DDP_IRQ_LAYER_FPS_ID, tag_name, index);
			}
		}
	}
#endif
