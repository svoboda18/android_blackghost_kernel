...
//#include <disp_dts_gpio.h> /* DTS GPIO */ 4.19
...
static void disp_pwm_query_backlight(char *debug_output)
{
	...
	if (high_width > 0) {
		...
		error = snprintf(temp_buf, buf_max_len, "backlight is on (%d), ddp_pwm power:(%d)",
			high_width, g_pwm_is_power_on);
	} else {
		error = snprintf(temp_buf, buf_max_len, "backlight is off, ddp_pwm power:(%d)",
			g_pwm_is_power_on);
	}
