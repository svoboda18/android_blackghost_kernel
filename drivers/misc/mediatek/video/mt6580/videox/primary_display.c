...
static void directlink_path_add_memory(struct WDMA_CONFIG_STRUCT *p_wdma)
{
	...
	int virtual_height = disp_helper_get_option(DISP_HELPER_OPTION_FAKE_LCM_HEIGHT);
	int virtual_width = disp_helper_get_option(DISP_HELPER_OPTION_FAKE_LCM_WIDTH);
			       ...
			       unsigned int userdata)
{
	...
	if (_should_update_lcm()){
		x = disp_helper_get_option(DISP_HELPER_OPTION_FAKE_LCM_X);
		if (x)
			DISPERR("error, lcm fake x is %d\n", x);
		y = disp_helper_get_option(DISP_HELPER_OPTION_FAKE_LCM_Y);
		if (y)
			DISPERR("error, lcm fake y is %d\n", y);

		width = disp_helper_get_option(DISP_HELPER_OPTION_FAKE_LCM_WIDTH);
		if (width == 0)
			DISPERR("error, lcm fake width is null\n");
		height = disp_helper_get_option(DISP_HELPER_OPTION_FAKE_LCM_HEIGHT);
		if (height == 0)
	...
	if (pgc->plcm->params->dsi.customization_esd_check_enable == 0) {
		...
		if (primary_display_is_video_mode()) {
			...
			if (primary_display_cmdq_enabled()) {
				...
				_primary_path_unlock(__func__);
				DISPDBG("[ESD]ESD check: EXT TE, video mode CMDQ flush done\n");
					 ...
					 int need_lock)
{
	...
	if (pgc->session_mode == DISP_SESSION_DIRECT_LINK_MODE
	    && sess_mode == DISP_SESSION_DECOUPLE_MODE) {
		/* dl to dc */
		DISPDBG("primary display DL_switch_to_DC_fast\n");
		...
		/* dc to dl */
		DISPDBG("primary display DC_switch_to_DL_fast\n");
...
int primary_display_resume(void)
{
...

	{
		...
		data_config->dst_h =
			disp_helper_get_option(DISP_HELPER_OPTION_FAKE_LCM_HEIGHT);
...
static int _primary_path_idlemgr_monitor_thread(void *data)
{
	...
	while (1) {
		...
		mmprofile_log_ex(ddp_mmp_get_events()->idlemgr,
			       MMPROFILE_FLAG_START, 0, 0);
		DISPDBG("primary enter idle state\n");
...
#else
//		disp_dts_gpio_select_state(DTS_GPIO_STATE_TE_MODE_GPIO); 4.19
#endif
		eint_flag++;
#endif
		if (eint_flag == 0)
...
//			disp_dts_gpio_select_state(DTS_GPIO_STATE_TE_MODE_TE); 4.19
#endif
#endif
	}
