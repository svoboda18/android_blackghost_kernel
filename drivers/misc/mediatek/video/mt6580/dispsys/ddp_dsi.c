	...
	if (state == CMDQ_BEFORE_STREAM_SOF) {
		...
		for (i = 0; i < 3; i++) {
			...
			if (dsi_i == 0) {
				...
				DISPDBG("polling dsi read ready done in CMDQ_ESD_CHECK_READ\n");
			}
			...
			if (dsi_i == 0) {
				...
				DISPDBG("polling dsi busy done in CMDQ_ESD_CHECK_READ\n");
			}
		...
		for (i = 0; i < 3; i++) {
			...
			if (packet_type == 0x1A || packet_type == 0x1C) {
				...
				DISPDBG("DSI read long packet size: %d\n", recv_data_cnt);
				if (recv_data_cnt <= 4) {
					memcpy((void *)&(dsi_params->lcm_esd_check_table[i].para_list[0]),
					(void *)&read_data1, recv_data_cnt);
				} else if (recv_data_cnt <= 8) {
					memcpy((void *)&(dsi_params->lcm_esd_check_table[i].para_list[0]),
					(void *)&read_data1, 4);
					memcpy((void *)&(dsi_params->lcm_esd_check_table[i].para_list[4]),
					...
					memcpy((void *)&(dsi_params->lcm_esd_check_table[i].para_list[4]),
						(void *)&read_data2, 4);
					memcpy((void *)&(dsi_params->lcm_esd_check_table[i].para_list[8]),
