...
static __s32 g_x_roomt[ENUM_MAX] = { 0 };
...
int tscpu_thermal_clock_on(void)
{
	...
	ret = enable_clock(MT_CG_THEM_SW_CG, "THERMAL");	/* [mt6580]Fix ME */
...
int tscpu_thermal_clock_off(void)
{
	...
	ret = disable_clock(MT_CG_THEM_SW_CG, "THERMAL");	/* [mt6580]Fix ME */
...
static __s32 temperature_to_raw_room(__u32 ret)
{
	...
	__s32 format_1 = 0;
	__s32 format_2 = 0;
	__s32 format_3[ENUM_MAX] = { 0 };
	__s32 format_4[ENUM_MAX] = { 0 };
...
int (*get_immediate_tsX[ENUM_MAX])(void) = {
...
static int read_tc_raw_and_temp(volatile u32 *tempmsr_name, ts_e ts_name, //thermal_sensor_name -> ts_e
				int *ts_raw)
{
	int temp = 0, raw = 0;
...
{
	/*
	   Yes, the software reset was not specific requested on mt6580.
	   ...
	   The specific software reset was requested for the later projects (Ex. mt6735, mt6797/97T, K.)
	 */

#if 0				/* mt6580's TC has no reset function */
	int temp = 0;
	/* reset thremal ctrl */
