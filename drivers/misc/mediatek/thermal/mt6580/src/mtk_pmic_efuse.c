...
static void mtktspmic_read_efuse(void)
{
	...
	ret = pmic_read_interface(0x063A, &efuse_val0, 0xFFFF, 0);
	if (ret) {
		mtktspmic_info("[%s] Failed to read pmic register 0x063A",
			__func__);
		return;
	}
	ret = pmic_read_interface(0x063C, &efuse_val1, 0xFFFF, 0);
	if (ret) {
		mtktspmic_info("[%s] Failed to read pmic register 0x063C",
			__func__);
		return;
	}
	mtktspmic_info("PMIC_Debug %d\n", __LINE__);
