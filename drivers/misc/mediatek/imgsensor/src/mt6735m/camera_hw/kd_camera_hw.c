...
{
	int ret = 0;
	pr_debug("mtkcam_gpio_init[kd_camera_hw.c] %p ,\n", pdev);
	camctrl = devm_pinctrl_get(&pdev->dev);
	if (IS_ERR(camctrl)) {
		dev_err(&pdev->dev, "Cannot find camera pinctrl!");
		pr_debug("mtkcam_gpio_init[kd_camera_hw.c] %p %llx, %s\n", pdev, camctrl, pdev->name);
		ret = PTR_ERR(camctrl);
		return -1;
	}
...
#define VOL_1000 1000000
	...
	if ((currSensorName && (0 == strcmp(SENSOR_DRVNAME_OV5647_MIPI_RAW, currSensorName)))||
				(currSensorName && (0 == strcmp(SENSOR_DRVNAME_OV5670_MIPI_RAW, currSensorName))))
	{
		printk("ov5647 power on\n");
		...
		if (0 == strcmp(SENSOR_DRVNAME_OV5647_MIPI_RAW, currSensorName)){
			if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN])
		    ...
		    if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN])
				mtkcam_gpio_set(pinSetIdx, CAMPDN, pinSet[pinSetIdx][IDX_PS_CMPDN + IDX_PS_OFF]);
			mdelay(5);
		}else if (0 == strcmp(SENSOR_DRVNAME_OV5670_MIPI_RAW, currSensorName)){
			if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN])
		//	mtkcam_gpio_set(pinSetIdx, CAMPDN, pinSet[pinSetIdx][IDX_PS_CMPDN + IDX_PS_ON]);
				mtkcam_gpio_set(pinSetIdx, CAMPDN, pinSet[pinSetIdx][IDX_PS_CMPDN + IDX_PS_OFF]);
			mdelay(5);
	...
	else if ((pinSetIdx == 1) && currSensorName && (0 == strcmp(SENSOR_DRVNAME_GC2385_MIPI_RAW, currSensorName))) {
	...
	else if ((currSensorName) &&(0 == strcmp(SENSOR_DRVNAME_BF2253L_MIPI_RAW, currSensorName)))
    {
      ...
      else if ((pinSetIdx == 1)&& (currSensorName) &&(0 == strcmp(SENSOR_DRVNAME_GC030A_MIPI_RAW, currSensorName)))
    {
        printk("luoyun PowerOn BF2253L main  pinSetIdx=%d currSensorName=%s \n",pinSetIdx,currSensorName);
        if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN])
                mtkcam_gpio_set(pinSetIdx, CAMPDN, pinSet[pinSetIdx][IDX_PS_CMPDN + IDX_PS_ON]);
	...
	else if((currSensorName&& (0 == strcmp(SENSOR_DRVNAME_BF20A1MIPI_RAW, currSensorName)))) {
		/* First Power Pin low and Reset Pin Low */
		if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN])
			mtkcam_gpio_set(pinSetIdx, CAMPDN,pinSet[pinSetIdx][IDX_PS_CMPDN + IDX_PS_ON]);
 ...
 else if((currSensorName&& (0 == strcmp(SENSOR_DRVNAME_GC02M1_MIPI_RAW, currSensorName)))) {
                /* First Power Pin low and Reset Pin Low */
                if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN])
                        mtkcam_gpio_set(pinSetIdx, CAMPDN,pinSet[pinSetIdx][IDX_PS_CMPDN + IDX_PS_ON]);
	...
	else if ((currSensorName&& (0 == strcmp(SENSOR_DRVNAME_GC5035_MIPI_RAW, currSensorName)))||
		(currSensorName&& (0 == strcmp(SENSOR_DRVNAME_GC5025_MIPI_RAW, currSensorName)))) {
		/* First Power Pin low and Reset Pin Low */
		if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN])
			mtkcam_gpio_set(pinSetIdx, CAMPDN,pinSet[pinSetIdx][IDX_PS_CMPDN + IDX_PS_OFF]);
	...
	else if ( (currSensorName) &&(0 == strcmp(SENSOR_DRVNAME_GC030A_MIPI_RAW, currSensorName)))
        {
                printk("luoyun PowerDown BF2253L main pinSetIdx=%d currSensorName=%s \n",pinSetIdx,currSensorName);
                //Set Power Pin low and Reset Pin Low
                if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN])
