...
#define PFX "HI2907_SensroDriver"
...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = HI2907MIPI_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h

    .checksum_value = 0x22ac53f7,//0x4860a7af,        //checksum value for Camera Auto Test
	...
	.sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_Gr,//sensor output first pixel color SENSOR_OUTPUT_FORMAT_RAW_Gr
...
static imgsensor_struct imgsensor = {
    ...
    .min_frame_length = 1224,//add zsf cista
...
static void set_max_framerate(UINT16 framerate,kal_bool min_framelength_en)
{
    ...
    LOG_INF(">>> set_max_framerate():framerate = %d, min framelength should enable = %d\n", framerate,min_framelength_en);
...
static void set_shutter(kal_uint16 shutter)
{
	...
	LOG_INF("set_shutter(): shutter =%d, framelength =%d,mini_framelegth=%d\n", shutter,imgsensor.frame_length,imgsensor.min_frame_length);
}
/*************************************************************************
* FUNCTION
*    set_gain
...
{
	LOG_INF("sensor_init begin");
	
	/*system*/
	write_cmos_sensor(0x0103,0x01);
	write_cmos_sensor(0x0100,0x00);		
	//mirror
	write_cmos_sensor(0x0101, 0x03);// ; NO FLIP   0x03
	write_cmos_sensor(0x3009,0x03); 
...
static void preview_setting(kal_uint16 currefps)
{
  ...
  LOG_INF("E! preview_setting currefps:%d\n",currefps);
...
{
    LOG_INF("E! capture_setting currefps:%d\n",currefps);
        ...
        do {
            *sensor_id = ((read_cmos_sensor(0x0000) << 8) | read_cmos_sensor(0x0001));
            if (*sensor_id == imgsensor_info.sensor_id) {
                LOG_INF("get_imgsensor_id OK,i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
            LOG_INF("get_imgsensor_id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            sensor_id = ((read_cmos_sensor(0x0000) << 8) | read_cmos_sensor(0x0001));
            if (sensor_id == imgsensor_info.sensor_id) {
                LOG_INF("open:Read sensor id OK,i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
                break;
            }
...
static kal_uint32 set_max_framerate_by_scenario(enum MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
    ...
    LOG_INF(">>>>set_max_framerate_by_scenario():scenario_id = %d, framerate = %d\n", scenario_id, framerate);
