...
#include "bf20a1mipi_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "bf20a1_camera_sensor"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = BF20A1_SENSOR_ID,	//0x20a1	//record sensor id defined in Kd_imgsensor.h

	.checksum_value = 0xf7375923,        //checksum value for Camera Auto Test

	.pre = {
		.pclk = 12000000,                //record different mode's pclk
		.linelength = 800,                //record different mode's linelength
...
/* SENSOR FLIP */
#define BF20A1_HFLIP_EN    0
#define BF20A1_VFLIP_EN    0
...
static void sensor_init(void)
{
	...
	write_cmos_sensor( 0xe0, 0x06 );//MIPI CLK 120M
	...
	write_cmos_sensor( 0x6b, 0x03 );
	write_cmos_sensor( 0x6c, 0x84 );
	write_cmos_sensor( 0x73, 0x04 );//for mipi
	write_cmos_sensor( 0x00, (0x10 | (BF20A1_HFLIP_EN << 1) | BF20A1_VFLIP_EN));	
...
{
	LOG_INF("BF20A1 preview setting\n");
	write_cmos_sensor( 0xe8, 0x10 );
	write_cmos_sensor( 0x5d, 0xff );	
}    /*    preview_setting  */

static void capture_setting(kal_uint16 currefps)
{
	LOG_INF("BF20A1 capture setting ,currefps:%d\n",currefps);
	write_cmos_sensor( 0xe8, 0x10 );
	write_cmos_sensor( 0x5d, 0xff );	
}
...
{
	LOG_INF("BF20A1 normal_video setting currefps:%d\n",currefps);
	write_cmos_sensor( 0xe8, 0x10 );	
	write_cmos_sensor( 0x5d, 0xff );	
...
{
	LOG_INF("BF20A1 hs_video_setting\n");
	write_cmos_sensor( 0xe8, 0x10 );
	write_cmos_sensor( 0x5d, 0xff );	
}
...
{
	LOG_INF("BF20A1 slim_video_setting\n");
	write_cmos_sensor( 0xe8, 0x10 );
	write_cmos_sensor( 0x5d, 0xff );	
}
...
{
	LOG_INF("test_pattern_mode enable: %d\n", enable);
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("bf20a1 get_imgsensor_id i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
				return ERROR_NONE;
			}
			LOG_INF("bf20a1 get_imgsensor_id Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) 
	{
		...
		do {
			...
			if (sensor_id == imgsensor_info.sensor_id) 
			{
				LOG_INF("bf20a1 open i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
				break;
			}
			LOG_INF("bf20a1 open Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
			retry--;
		} while(retry > 0);
...
{//This Function not used after ROME
	LOG_INF("BF20A1 set_video_mode :framerate = %d\n ", framerate);
...
{
	LOG_INF("BF20A1 set_auto_flicker_mode:enable = %d, framerate = %d \n", enable, framerate);
	spin_lock(&imgsensor_drv_lock);
...
UINT32 BF20A1_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc!=NULL)
		*pfFunc=&sensor_func;
	return ERROR_NONE;
}    /*    BF20A1MIPI_RAW_SensorInit    */
