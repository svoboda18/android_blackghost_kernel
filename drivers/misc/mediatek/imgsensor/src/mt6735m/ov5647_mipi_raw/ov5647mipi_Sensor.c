...
static imgsensor_info_struct imgsensor_info = {
    ...
    .checksum_value = 0xb2000fed,        //checksum value for Camera Auto Test
...
{
    LOG_INF("OV5647_Sensor_Init_2lane E\n");
    //@@ global setting																  
...
{
    LOG_INF(" OV5647PreviewSetting_2lane enter\n");
	write_cmos_sensor(0x0100,0x00);      
	write_cmos_sensor(0x3036, 0x64);
	write_cmos_sensor(0x3612, 0x5b);
	write_cmos_sensor(0x3618, 0x04);
...
{
	LOG_INF("OV5647CaptureSetting_2lane enter! \n");	
    write_cmos_sensor(0x0100,0x00);      
	write_cmos_sensor(0x3036, 0x64);
	write_cmos_sensor(0x3612, 0x5b);
	write_cmos_sensor(0x3618, 0x04);
        ...
        do {
            *sensor_id = return_sensor_id();
            if (*sensor_id == imgsensor_info.sensor_id) {
                LOG_INF("zxw i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
            LOG_INF("zxw Read sensor id fail, write id:0x%x id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            sensor_id = return_sensor_id();
			//sensor_id = imgsensor_info.sensor_id; //zxw for test
            if (sensor_id == imgsensor_info.sensor_id) {
                printk("zxw i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
                break;
            }
            printk("zxw Read sensor id fail, write id:0x%x id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        if (sensor_id == imgsensor_info.sensor_id)
...
//UINT32 OV5647_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
