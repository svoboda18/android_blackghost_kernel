...
//ITD:modify XLWLHBLEJ-137 by quan.chang 190701 start
#include <linux/kernel.h>
#include <linux/module.h>
#include <linux/device.h>
//ITD:modify XLWLHBLEJ-137 by quan.chang 190701 end
#include "kd_camera_typedef.h"
#include "kd_imgsensor.h"
#include "kd_imgsensor_define.h"
#include "kd_imgsensor_errcode.h"

#include "gc8034mainmipi_Sensor.h"
//ITD:modify XLESHBLEL-608 by quan.chang 181112 start
//wangzhncai extern int main2callbacknextloop;
unsigned int islowlight=0;
//wangzhncai extern int isMainOpen;
//ITD:modify XLESHBLEL-608 by quan.chang 181112 end
/****************************Modify Following Strings for Debug****************************/
#define PFX "gc8034main_camera_sensor"
#define LOG_1 LOG_INF("GC8034MAIN,MIPI 2LANE\n")
/****************************   Modify end    *******************************************/
#define GC8034_DEBUG
...
//ITD:Modify Camera Perf Opti 20180831 Start
extern void kdSetI2CSpeed(u32 i2cSpeed);
//ITD:modify XLWLHBLEJ-137 by quan.chang 190701 start
int main_camera_flag_1 = 0;
int sub_camera_flag = 0;
//ITD:modify XLWLHBLEJ-137 by quan.chang 190701 end
//ITD:Modify Camera Perf Opti 20180831 End
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC8034MAIN_SENSOR_ID,       /*record sensor id defined in Kd_imgsensor.h*/
	...
	.cap = {
		...
		.mipi_data_lp2hs_settle_dc = 85, /*unit, ns*/
		.mipi_pixel_rate = 257200000,//257280000
		.max_framerate = 300,
	},
	.cap1 = {
...
//ITD:modify BAEGHBLEQ-564 by quan.chang 190423 start
	.isp_driving_current = ISP_DRIVING_6MA,                 /* mclk driving current */
//ITD:modify BAEGHBLEQ-564 by quan.chang 190423 end
...
static kal_uint16 read_cmos_sensor(kal_uint32 addr)
{
	...
	iReadRegI2CWithCamIndex(0, pu_send_cmd, 1, (u8 *)&get_byte, 1, imgsensor.i2c_write_id);
	return get_byte;
}
...
static void write_cmos_sensor(kal_uint32 addr, kal_uint32 para)
{
	...
	iWriteRegI2CWithCamIndex(0,pu_send_cmd, 2, imgsensor.i2c_write_id);
}
...
static void gc8034_gcore_read_otp_info(void)
{
	...
	pr_info("GC8034_OTP_DD : flag_dd = 0x%x\n", flagdd);
	...
	flag_af = gc8034_read_otp(3, 0x34);
	...
	for (index = 0; index < 2; index++) {
		...
		case 0x00:
			...
			break;
		case 0x04:
			pr_info("GC8034_OTP_INFO group %d is Valid !!\n", index + 1);
			check = 0;
			...
			if ((check % 255 + 1) == info[INFO_WIDTH - 1]) {
				...
				pr_info("GC8034_OTP_INFO Check sum %d Error !!\n", index + 1);
			break;
		case 0x08:
		case 0x0c:
		...
		case 0x00:
			...
			break;
		case 0x04:
			pr_info("GC8034_OTP_WB group %d is Valid !!\n", index + 1);
			check = 0;
			...
			if ((check % 255 + 1) == wb[WB_WIDTH - 1]) {
				...
				pr_info("GC8034_OTP_WB Check sum %d Error !!\n", index + 1);
			break;
		case 0x08:
		case 0x0c:
		...
		case 0x00:
			...
			break;
		case 0x40:
			pr_info("GC8034_OTP_GOLDEN group %d is Valid !!\n", index + 1);
			check = 0;
			...
			if ((check % 255 + 1) == golden[GOLDEN_WIDTH - 1]) {
				...
				pr_info("GC8034_OTP_GOLDEN Check sum %d Error !!\n", index + 1);
		...
		case 0x00:
			...
			break;
		case 0x04:
			pr_info("GC8034_OTP_AF group %d is Valid !!\n", index + 1);
			check = 0;
			...
			if ((check % 255 + 1) == af[AF_WIDTH - 1]) {
				otp_af.af_read_success=1;
				otp_af.af_inf_H = af[0];
				otp_af.af_inf_L = af[1];
				otp_af.af_med_H = af[2];
				otp_af.af_med_L = af[3];
				otp_af.af_mac_H = af[4];
				otp_af.af_mac_L = af[5];
				gc8034_otp_info.af_infinity = ((af[0] << 8) & 0xff00) + af[1];
				gc8034_otp_info.af_macro = ((af[4] << 8) & 0xff00) + af[5];
				gc8034_otp_info.af_flag = gc8034_otp_info.af_flag | 0x01;
			} else
				pr_info("GC8034_OTP_AF Check sum %d Error !!\n", index + 1);
			break;
		case 0x08:
		case 0x0c:
	...
	for (index = 0; index < 2; index++) {
		...
		case 0x00:
			...
			break;
		case 0x04:
			pr_info("GC8034_OTP_LSC group %d is Valid !\n", index + 1);
			check = 0;
			if (index == 0) {
			...
			if ((check % 255 + 1) == lsc_checksum)
				...
				pr_info("GC8034_OTP_LSC check sum error, check sum read : 0x%x, calculate:0x%x!!\n", lsc_checksum, check % 255 + 1);
			}
			break;
		case 0x08:
		case 0x0c:
	...
	pr_info("GC8034_OTP_INFO:module_id=0x%x\n", gc8034_otp_info.module_id);
	pr_info("GC8034_OTP_INFO:lens_id=0x%x\n", gc8034_otp_info.lens_id);
	pr_info("GC8034_OTP_INFO:vcm_id=0x%x\n", gc8034_otp_info.vcm_id);
	pr_info("GC8034_OTP_INFO:vcm_driver_id=0x%x\n", gc8034_otp_info.vcm_driver_id);
	pr_info("GC8034_OTP_INFO:data=%d-%d-%d\n", gc8034_otp_info.year, gc8034_otp_info.month, gc8034_otp_info.day);
	pr_info("GC8034_OTP_WB:r/g=0x%x\n", gc8034_otp_info.rg_gain);
	pr_info("GC8034_OTP_WB:b/g=0x%x\n", gc8034_otp_info.bg_gain);
	pr_info("GC8034_OTP_GOLDEN:golden_rg=0x%x\n", gc8034_otp_info.golden_rg);
	pr_info("GC8034_OTP_GOLDEN:golden_bg=0x%x\n", gc8034_otp_info.golden_bg);
	pr_info("GC8034_OTP_AF:infitiy=0x%x\n", gc8034_otp_info.af_infinity);
	pr_info("GC8034_OTP_AF:macro=0x%x\n", gc8034_otp_info.af_macro);
#endif
...
static void gc8034_otp_function(void)
{
	...
	for (i = 0; i < GC8034_OTP_ID_SIZE; i++)
		if (otp_id[i] != gc8034_otp_info.otp_id[i]) {
			pr_info("gc8034_otp_function otp_id %d mismatch(0x%x!=0x%x), re-read otp",i,gc8034_otp_info.otp_id[i],otp_id[i]);
			gc8034_gcore_read_otp_info();
			break;
		}
...
//ITD:modify XLWLHBLEJ-338 by quan.chang 190703 start
#ifdef CONFIG_TRAN_SYSTEM_DEVINFO
extern void app_get_back_sensor_name(char *back_name);
extern void app_get_main2_sensor_name(char *back_name);
#endif
//ITD:modify XLWLHBLEJ-338 by quan.chang 190703 end
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				gc8034_otp_identify();
				pr_info("[gc8034_maincamera_sensor]get_imgsensor_id:i2c write id: 0x%x, sensor id: 0x%x\n",
							imgsensor.i2c_write_id, *sensor_id);
//ITD:modify XLWLHBLEJ-137 by quan.chang 190701 start
				main_camera_flag_1 = 1;
//ITD:modify XLWLHBLEJ-137 by quan.chang 190701 end
//ITD:modify XLWLHBLEJ-338 by quan.chang 190703 start
				#ifdef CONFIG_TRAN_SYSTEM_DEVINFO
				app_get_back_sensor_name("gc8034_17201662");
				app_get_main2_sensor_name("gc6153_17201581");
				#endif
//ITD:modify XLWLHBLEJ-338 by quan.chang 190703 end
				return ERROR_NONE;
			}
			pr_info("[gc8034_maincamera_sensor]get_imgsensor_id:Read sensor id fail, write id: 0x%x, id: 0x%x\n",
						imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
			if (sensor_id == imgsensor_info.sensor_id) {
				pr_info("[gc8034_maincamera_sensor]open:i2c write id: 0x%x, sensor id: 0x%x\n",
							imgsensor.i2c_write_id, sensor_id);
				break;
			}
			pr_info("[gc8034_maincamera_sensor]open:Read sensor id fail, write id: 0x%x, id: 0x%x\n",
						imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
static kal_uint32 close(void)
{
	...
	streaming_control(0);
//ITD:modify XLESHBLEL-608 by quan.chang 181112 start
   //wangzhncai main2callbacknextloop = 0;
   //wangzhncai isMainOpen = 0;
//ITD:modify XLESHBLEL-608 by quan.chang 181112 end
	/*No Need to implement this function*/
	return ERROR_NONE;
}
...
UINT32 GC8034MAIN_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
...
//ITD:modify XLWLHBLEJ-137 by quan.chang 190701 start
static unsigned int tran_camera_info_level_1=0;
static struct class *camera_info_class = NULL;
static struct device *camera_info_dev = NULL;
static struct device_attribute dev_attr_camera_info_level;

static ssize_t camera_info_level_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{

	if((1 == main_camera_flag_1) && (1 == sub_camera_flag)) {
		tran_camera_info_level_1 = 3;
	}
	else if((1 == main_camera_flag_1) && (0 == sub_camera_flag)) {
		tran_camera_info_level_1 = 2;
	}
	else if((0 == main_camera_flag_1) && (1 == sub_camera_flag)) {
		tran_camera_info_level_1 = 1;
	}
	else {
		tran_camera_info_level_1 = 0;
	}
    printk("get camera_info_level_show value is:%d \n",tran_camera_info_level_1);
	return sprintf(buf, "%u\n", tran_camera_info_level_1);
}

static ssize_t camera_info_level_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	tran_camera_info_level_1 = mode;
	printk("ludesuo camera_info_level_store mode=%d,tran_camera_info_level_1=%d \n", mode,tran_camera_info_level_1);

   	return size;
}

//static DEVICE_ATTR(camera_info_level, 0666, camera_info_level_show, camera_info_level_store);
static DEVICE_ATTR_RW(camera_info_level);
// sys/class/tran_camera_info/tran_camera_info/
static int __init tran_camera_info_level_1_init(void)
{
	printk("start\n");
	camera_info_class = class_create(THIS_MODULE, "tran_camera_info");
	if (IS_ERR(camera_info_class))
	{
		printk("Unable to create class, err = %d\n", (int)PTR_ERR(camera_info_class));
		return 0;
	}
	camera_info_dev = device_create(camera_info_class, NULL, 0, 0,"tran_camera_info");
	if(NULL == camera_info_dev)
	{
		printk("device_create fail\n");
		return 0;
	}

	device_create_file(camera_info_dev, &dev_attr_camera_info_level);

	printk("Done\n");
	return 0;
}

static void __exit tran_camera_info_level_1_exit(void)
{
	device_remove_file(camera_info_dev, &dev_attr_camera_info_level);
	device_unregister(camera_info_dev);
	if(camera_info_class!=NULL)
	{
		class_destroy(camera_info_class);
	}
}


module_init(tran_camera_info_level_1_init);
module_exit(tran_camera_info_level_1_exit);
