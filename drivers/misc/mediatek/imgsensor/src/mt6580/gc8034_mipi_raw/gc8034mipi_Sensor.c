...
//ITD:modify XLWLHBLEJ-137 by quan.chang 190701 end
				#ifdef CONFIG_TRAN_SYSTEM_DEVINFO
				app_get_front_sensor_name("gc8034_17201636");
				#endif
				return ERROR_NONE;
			}
			pr_info("[gc8034_camera_sensor]get_imgsensor_id:Read sensor id fail, write id: 0x%x, id: 0x%x\n",
						imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
