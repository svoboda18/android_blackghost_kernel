...
static struct imgsensor_info_struct imgsensor_info = {
    ...
    .checksum_value = 0x740fceb2,        //checksum value for Camera Auto Test
...
static void sensor_init(void)
{
    ...
    */
    ...
    // Horizontal binning
    write_cmos_sensor(0x3820, 0x0E); // flip off, v bin off 08
    write_cmos_sensor(0x3821, 0x01); // mirror on, h bin on 07
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            *sensor_id = return_sensor_id();
            ...
            printk("ov5648 Read sensor id fail, write id:0x%x id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
