...
int iReadRegI2CWithCamIndex(u32 index, u8 *a_pSendData, u16 a_sizeSendData, u8 *a_pRecvData, u16 a_sizeRecvData,
		u16 i2cId)
{
	int i4RetValue = 0;
	if (index == 0) {
...
int iWriteRegI2CWithCamIndex(u32 index, u8 *a_pSendData, u16 a_sizeSendData, u16 i2cId)
{
	int i4RetValue = 0;
	int retry = 3;
			...
			if (DUAL_CAMERA_NONE_SENSOR != g_invokeSocketIdx[i]) {
				...
				if (sensorID == 0) {	/* not implement this feature ID */
					...
					if (pmtk_ccm_name == NULL) {
						PK_DBG("mtk ccm name not found");
					}

					pg_invokeSensorNameStr = strchr(g_invokeSensorNameStr[i], '\0');

					if (pmtk_ccm_name && pg_invokeSensorNameStr) {
