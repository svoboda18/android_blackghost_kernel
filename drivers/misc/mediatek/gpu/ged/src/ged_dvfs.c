...
extern void (*mtk_get_gpu_dvfs_from_fp)(enum MTK_GPU_DVFS_TYPE* peType, unsigned long *pulFreq);
	...
	unsigned int cur_freq_khz, bool bFB)
{
	...
	/* TO-DO: On mt6771, this seems no need, since we could get MB already
...
void ged_dvfs_track_latest_record(enum MTK_GPU_DVFS_TYPE *peType, unsigned long *pulFreq)
{
	*peType = g_CommitType;
