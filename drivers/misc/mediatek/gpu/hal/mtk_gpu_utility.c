...
//-----------------------------------------------------------------------------
void (*mtk_get_gpu_dvfs_from_fp)(enum MTK_GPU_DVFS_TYPE* peType, unsigned long *pulFreq) = NULL;
EXPORT_SYMBOL(mtk_get_gpu_dvfs_from_fp);

bool mtk_get_gpu_dvfs_from(enum MTK_GPU_DVFS_TYPE* peType, unsigned long *pulFreq)
{
