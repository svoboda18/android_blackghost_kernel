...
#ifdef CONFIG_TRUSTKERNEL_TEE_SECURE_PMIC_WRAP
#include <linux/tee_secure_timer.h>

#define pmic_cpufreq_write(addr, val) \
    tkcore_pmic_sec_write((void *) ((addr) - PWRAP_BASE_ADDR), val)
#endif
...
void mt_cpufreq_set_pmic_phase(enum pmic_wrap_phase_id phase)
{
 ...
 #ifdef CONFIG_TRUSTKERNEL_TEE_SECURE_PMIC_WRAP
       pmic_cpufreq_write(pw.addr[i].cmd_addr, pw.set[phase]._[i].cmd_addr);
       pmic_cpufreq_write(pw.addr[i].cmd_wdata, pw.set[phase]._[i].cmd_wdata);
 #else
...
void mt_cpufreq_set_pmic_cmd(enum pmic_wrap_phase_id phase, int idx, unsigned int cmd_wdata)
{
	...
	if (pw.phase == phase)
#ifdef CONFIG_TRUSTKERNEL_TEE_SECURE_PMIC_WRAP
       pmic_cpufreq_write(pw.addr[idx].cmd_wdata, cmd_wdata);
#else
