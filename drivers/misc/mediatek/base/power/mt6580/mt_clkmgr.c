...
void dump_clk_info_by_id(enum cg_clk_id id)
{
	...
	if (NULL != clk) {
		...
		pr_info("[%d,\t%d,\t%s,\t"HEX_FMT",\t%s,\t%s,\t%s]\n",
			...
			clk->name,
			(clk_src < NR_CLKS)) ? clks[clk_src].name : "MT_CG_INVALID";
	}
}
