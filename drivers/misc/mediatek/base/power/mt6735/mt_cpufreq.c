...
static int _mt_cpufreq_cpu_CB(unsigned int action,
				unsigned int hcpu);
...
/* Procedure of hotplug has changed in Kernel-4.19
 * Hence no need this method now .
 */
/*
				...
				unsigned int cpu)
{
	...
	pr_info("@%s():%d, cpu = %d, action = %lu, oppidx = %d, num_online_cpus = %d\n",
 ...
 *	Hotplug callback for dynamic online
 *	arg : cpu (cpu being online)
 ...
 *	Hotplug callback for dynamic offline for cpu
 *	arg : cpu (cpu being offline)
 *	returns: 0 in case of success . 
 ***********************************************/
...
#endif
	/* kernel-4.19-lc  has removed this
	 * instead need to use dynamic hotplug interface
	 * to get CPU state of online and offline
	 */
