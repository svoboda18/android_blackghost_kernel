...
uint32_t g_cmd_sn = 0;
struct input_dev* fp_input_dev;
//extern char *fingerprint_name_info;
int sunritel_fp_compatible_flag = 0;
...
static int fp_report_key_event(struct input_dev* input){
	int rc = 0;
	fpsensor_debug(DEBUG_LOG, "fp_report_key_event 123456 \n");
...
//------------------------------ree
        ...
        if (IS_ERR(fpsensor->fp_cs_low)) {
		 ...
		 fpsensor->fp_cs_set = pinctrl_lookup_state(fpsensor->pinctrl1, "spi_cs_set");
...
static irqreturn_t fpsensor_irq(int irq, void *handle)
{
	...
	fpsensor_debug(DEBUG_LOG, "fpsensor_irq 123456 \n");
	fp_report_key_event(fp_input_dev);
...
static int fpsensor_init_input(void)
{
	int error = 0;
	fp_input_dev = input_allocate_device();
	if (!fp_input_dev) 
	{
		fpsensor_debug(ERR_LOG ,"input_allocate_device(..) fail\n");
		return (-ENOMEM);
	}
	fp_input_dev->name = "fpsensor_dev-keys";
	__set_bit(EV_KEY  , fp_input_dev->evbit );
	__set_bit(KEY_F11, fp_input_dev->keybit);
		
	error = input_register_device(fp_input_dev);
	if (error) 
	{
		fpsensor_debug(ERR_LOG ,"input_register_device fail\n");
		input_free_device(fp_input_dev);
		fp_input_dev = NULL;
		return (-ENODEV);
	}
	return error;
...
static int fpsensor_probe(struct spi_device *spi)
{
    ...
    FUNC_ENTRY();
	if(sunritel_fp_compatible_flag == 1)
	{
		return -ENODEV;
	}
...
//    fingerprint_name_info = "ICNF7332";
