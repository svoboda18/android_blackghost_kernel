...
void mas_set_spi_controller_data(struct spi_device *spi)
{
	spi->mode            = SPI_MODE_0;
	spi->bits_per_word   = 8;
...
unsigned int mas_get_irq(struct device *dev){
 /*   int irq_num;
	...
	MALOGE("mas_get_irq finger_int_pin:%d",finger_int_pin);
    gpio_direction_input(finger_int_pin);
    irq_num = gpio_to_irq(finger_int_pin);
	MALOGE("mas_get_irq irq_num:%d",irq_num); */
//*******************************************************

/*
    node = of_find_matching_node(node, sof_dev_match);
    if (NULL == node) {
        MALOGE("Can not find sof eint device node!");
        return -ENODATA;
    }
