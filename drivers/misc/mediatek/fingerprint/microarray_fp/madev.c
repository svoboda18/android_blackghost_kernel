...
static int init_interrupt(void)
{
	...
	MALOGE("init_interrupt start");
    irq = mas_get_irq(&smas->spi->dev);
    if(irq<=0){
        ret = irq;
        MALOGE("mas_get_irq");
    }
	MALOGE("init_interrupt start irq:%d",irq);
#ifdef DOUBLE_EDGE_IRQ
    ...
    enable_irq_wake(irq);
	MALOGE("init_interrupt end ret:%d",ret);
    return ret;
}
...
int mas_probe(struct spi_device *spi) {
    ...
    MALOGD("20201125__mas_probe_start");
...
#endif
#ifdef CONFIG_MTK_HARDWAREINFO
    fingerprint_name_info = "microarray_fp";
#endif
