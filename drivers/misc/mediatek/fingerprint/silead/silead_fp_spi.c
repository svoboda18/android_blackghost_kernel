...
#include <mtk_spi.h>    //mt_chip_conf定义一般在drivers/spi/mediatek/$(MTK_PLATFORM)中, mt6735 kernel-3.18需要#include <mt_spi.h>
#include "silead_fp.h"
...
#ifndef SPI_CLK
#define SPI_CLK 100*1000*1000 /* Default assume 100Mhz */
#endif /* !SPI_CLK */

#define MHZ (1000*1000)
#define SPI_MIN_SPEED (1*MHZ)
#define SPI_MAX_SPEED (25*MHZ)
...
static struct mt_chip_conf m_chip_config = {
    .setuptime    = 3,
    .holdtime     = 3,
    .high_time    = 14,
    .low_time     = 14,
...
static int32_t sl_fp_spi_set_speed(struct spi_device * spi, uint32_t speed)
{
    if ((speed >= SPI_MIN_SPEED) && (speed <= SPI_MAX_SPEED)) {
        m_chip_config.high_time    = SPI_CLK/(2*speed);
        m_chip_config.low_time     = SPI_CLK/(2*speed);
        if ( m_chip_config.high_time < 1) {
             m_chip_config.high_time= 1;
        }
        if ( m_chip_config.low_time < 1) {
             m_chip_config.low_time = 1;
        }
		LOG_MSG_DEBUG(INFO_LOG, "silead_spi set_speed high_time = %d, low time = %d", m_chip_config.high_time, m_chip_config.low_time);
    } else {
        LOG_MSG_DEBUG(ERR_LOG, "Unsupport speed %d", speed);
    }
    return 0;
}
...
silfp_ree_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    case SPI_IOC_WR_MODE32:
        ...
        if (retval == 0) {
            u32    save = spi->mode;
            printk("silead_spi spi->mode =%x\n", spi->mode);
            if (tmp & ~SPI_MODE_MASK) {
				printk("silead_spi tmp =%x, return!!!!!", tmp);
                retval = -EINVAL;
                break;
            }
			...
			printk("silead_spi tmp =%x, spi mode mask 0x%x\n", tmp, SPI_MODE_MASK);
        }
        break;
    ...
    case SPI_IOC_WR_MAX_SPEED_HZ:
        ...
        if (retval == 0) {
		    ...
		    sl_fp_spi_set_speed(spi, tmp);
            retval = spi_setup(spi);
            if (retval >= 0)
                ree_data->speed_hz = tmp;
				//ree_data->speed_hz = 2*1000*1000;
            else
                dev_dbg(&spi->dev, "%d Hz (max)\n", tmp);
			printk("silead_spi  %d Hz (max)\n", tmp);
...
static int silfp_ree_probe(struct spi_device *spi)
{
    ...
    ree_data->spi = spi;
    spi->mode = 0;
	spi->controller_data = (void *)&m_chip_config;
	if (spi_setup(spi) != 0)
	{
		LOG_MSG_DEBUG(ERR_LOG,"%s: spi setup failed!\n", __func__);
		goto free_silfp;
	}
