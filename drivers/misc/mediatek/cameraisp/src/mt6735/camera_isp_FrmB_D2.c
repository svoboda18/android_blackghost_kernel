...
#define IRQ_LOG_KEEPER(irq, ppb, logT, fmt, ...) do {\
	...
	if (avaLen > 1) { \
		if (snprintf(pDes, avaLen, fmt, ##__VA_ARGS__) < 0) {\
			LOG_ERR("Error: snprintf fail");\
		} \
...
#define IRQ_LOG_PRINTER(irq, ppb_in, logT_in) do {\
	if (irq >= _IRQ_MAX) {\
		...
		if ((ppb < 0) || (ppb >= LOG_PPNUM) || (logT < 0) || (logT >= _LOG_MAX)) {\
			LOG_DBG("Error: Invalid Index ppb(%d) logT(%d)", ppb, logT);\
			break;\
		}\
...
static MINT32 ISP_RTBC_ENQUE_FRMB(MINT32 dma, ISP_RT_BUF_INFO_STRUCT_FRMB * prt_buf_info)
{
	...
	if ((rt_dma < 0) || (rt_dma >= _rt_dma_max_)) {
		LOG_ERR("Error: rt_dma(%d) Index is Invalid", rt_dma);
		Ret = -EFAULT;
		return Ret;
	}
	/* check max */
...
static MINT32 ISP_RTBC_DEQUE_FRMB(MINT32 dma, ISP_DEQUE_BUF_INFO_STRUCT_FRMB * pdeque_buf)
{
	...
	/*  */
	if ((rt_dma < 0) || (rt_dma >= _rt_dma_max_)) {
		LOG_ERR("rt_dma(%d) is Invalid Index", rt_dma);
		Ret = -EFAULT;
		return Ret;
	}
	if (IspInfo_FrmB.DebugMask & ISP_DBG_BUF_CTRL) {
...
static MINT32 ISP_ED_BufQue_CTRL_FUNC_FRMB(ISP_ED_BUFQUE_STRUCT_FRMB param)
{
	...
	case ISP_ED_BUFQUE_CTRL_ENQUE_FRAME:	/* signal that a specific buffer is enqueued */
		...
		if ((P2_EDBUF_RList_CurBufIdx < 0) || (P2_EDBUF_RList_CurBufIdx >= _MAX_SUPPORT_P2_FRAME_NUM_)) {
			LOG_ERR("Error: P2_EDBUF_RList_CurBufIdx(%d) is Invalid Index", P2_EDBUF_RList_CurBufIdx);
			ret = -EFAULT;
			return ret;
		}
		spin_lock(&(SpinLockEDBufQueList));
...
static MINT32 ISP_GET_MARKtoQEURY_TIME(ISP_WAIT_IRQ_STRUCT_FRMB * irqinfo)
{
	...
	if ((irqinfo->UserInfo.UserKey < 0) || (irqinfo->UserInfo.UserKey >= IRQ_USER_NUM_MAX)||
		(irqinfo->UserInfo.Type < 0) || (irqinfo->UserInfo.Type >= ISP_IRQ_TYPE_AMOUNT) ||
		(idx < 0) || (idx >= 32)) {
		LOG_DBG("Error: Invalid Index, irqinfo->UserInfo.UserKey(%d) irqinfo->UserInfo.Type(%d) idx(%d)",
...
static MINT32 ISP_WaitIrq_FrmB(ISP_WAIT_IRQ_STRUCT_FRMB * WaitIrq)
{
	...
	spin_lock_irqsave(&(IspInfo_FrmB.SpinLockIrq[eIrq]), flags);
	if ((idx < 0) || (idx >= 32)) {
		LOG_ERR("Error: idx(%d) Index is Invalid",idx);
		Ret = -EFAULT;
		goto EXIT;
	}
	...
	if ((int)(WaitIrq->UserInfo.Type) < (int)(ISP_IRQ_TYPE_INTB)
		...
		/* eis meta */
		if ((WaitIrq->UserInfo.Type < 0) ||
			(WaitIrq->UserInfo.Type >= ISP_IRQ_TYPE_INTB) ||
			(gEismetaRIdx < 0) ||
	...
	if (IrqStatus[ISP_IRQ_TYPE_INT] & ISP_IRQ_INT_STATUS_SOF1_INT_ST) {
		...
		}
		{
			...
			if ((gEismetaWIdx < 0) || (gEismetaWIdx >= EISMETA_RINGSIZE) ||
				(ISP_IRQ_TYPE_INT < 0) || (ISP_IRQ_TYPE_INT >= ISP_IRQ_TYPE_INTB)) {
				LOG_ERR("Error: Invalid Index");
			}
