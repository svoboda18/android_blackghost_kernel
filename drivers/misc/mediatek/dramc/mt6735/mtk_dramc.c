...
#ifdef APDMA_TEST
static ssize_t dram_dummy_read_test_show(struct device_driver *driver, char *buf)
{
...
static ssize_t dram_dummy_read_test_store(struct device_driver *driver,
				    const char *buf, size_t count)
{
	return count;
}
...
static ssize_t read_dram_temp_test_show(struct device_driver *driver, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "DRAM MR4 = 0x%x\n", read_dram_temperature());
}

static ssize_t read_dram_temp_test_store(struct device_driver *driver,
				    const char *buf, size_t count)
{
	return count;
}
...
#ifdef APDMA_TEST
DRIVER_ATTR_RW(dram_dummy_read_test);
#endif

#ifdef READ_DRAM_TEMP_TEST
DRIVER_ATTR_RW(read_dram_temp_test);
#endif
...
#ifdef FREQ_HOPPING_TEST
DRIVER_ATTR_RW(freq_hopping_test);
#endif
