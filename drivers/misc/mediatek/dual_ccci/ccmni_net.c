...
static void timer_func(struct timer_list *t)
{
	...
	struct ccmni_instance_t *ccmni = from_timer(ccmni, t, timer);
	int contin = 0;
	int ret = 0;
...
static int ccmni_create_instance(int md_id, int channel)
{
	...
	struct ccmni_v1_ctl_block_t *ctl_b = NULL;
	int has_write = 0;

	CHECK_MD_ID_RETURN_INT(md_id);
	ctl_b = (struct ccmni_v1_ctl_block_t *) ccmni_ctl_block[md_id];
	if (!ctl_b) {
...
{
	struct ccmni_v1_ctl_block_t *ctl_b = NULL;
	struct ccmni_instance_t *ccmni = NULL;
	...
	ccmni = ctl_b->ccmni_instance[channel];
