...
static void recover_md_wdt_irq(struct timer_list *unused);
static DEFINE_TIMER(md1_wdt_recover_timer, recover_md_wdt_irq);
...
static void ccci_get_platform_ver(char *ver, int len)
{
	int has_write = 0;

	if (!ver) {
		CCCI_ERR_INF(0, "cci", "%s:%d:ver is NULL",
			__func__, __LINE__);
		return;
	}

#ifdef ENABLE_CHIP_VER_CHECK
	has_write = snprintf(ver, len, "MT%04x_S%02x", mt_get_chip_hw_ver(), (mt_get_chip_hw_subcode() & 0xFF));
#else
	has_write = snprintf(ver, len, "MT6580_S00");
#endif
	if (has_write <= 0 || has_write >= len)
		CCCI_ERR_INF(0, "cci", "%s:%d:snprintf fail,has_write=%d",
...
int ccci_load_firmware_helper(int md_id, char img_err_str[], int len)
{
	...
	if (ccif_plat_drv == NULL) {
		...
		has_write = snprintf(img_err_str, len, "ccif_plat_drv == NULL");
		if (has_write <= 0 || has_write >= len)
...
int __init ccci_mach_init(void)
{
	...
	if ((ap_infra_base == 0) || (ap_mcu_reg_base == 0))
		get_dts_settings(0);
	ccci_get_platform_ver(ap_platform, sizeof(ap_platform));
