...
void platform_set_runtime_data(int md_id, struct modem_runtime_t *runtime)
{
	...
	has_write = snprintf(str, 16, "%s", CCCI_PLATFORM);
	...
	if (!runtime) {
		CCCI_ERR_INF(md_id, "ctl", "%s:%d:runtime is null",
			__func__, __LINE__);
		return;
	}
...
static int cfg_md_mem_layout(int md_id)
{
	...
	int ret = 0;
	phys_addr_t tmp_addr = 0;

	/* md_len = MD_IMG_RESRVED_SIZE + MD_RW_MEM_RESERVED_SIZE; */
	ret = get_md_resv_mem_info(md_id, &tmp_addr, &md_len, NULL, NULL);
	if (ret != 0) {
		CCCI_MSG_INF(md_id, "ctl",
			"%s:get_md_resv_mem_info fail,ret=%d\n", __func__, ret);
		return ret;
	}
	dsp_len = 0;
