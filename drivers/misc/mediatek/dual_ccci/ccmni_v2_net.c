...
static void timer_func(struct timer_list *t)
{
	...
	struct ccmni_v2_instance_t *ccmni = from_timer(ccmni, t, timer);
	int contin = 0;
	int ret = 0;
...
static void ccmni_v2_read(unsigned long arg)
{
	...
	for (; packet_cnt > 0; packet_cnt--) {
		...
		if (-CCCI_ERR_MEM_CHECK_FAIL ==
			...
			avai_in &= q_length - 1;
			if (avai_in < 0) {
				CCCI_ERR_INF(0, "cci", "%s:%d:avai_in=%d is invalid",
					__func__, __LINE__, avai_in);
				break;
			}
...
static int ccmni_v2_create_instance(int md_id, int channel)
{
	...
	struct dbg_info_ccmni_t *dbg_info;
#endif
	struct ccmni_v2_ctl_block_t *ctl_b = NULL;

	CHECK_MD_ID_RETURN_INT(md_id);
	ctl_b = (struct ccmni_v2_ctl_block_t *) ccmni_ctl_block[md_id];
	if (!ctl_b) {
		CCCI_ERR_INF(md_id, "net", "%s:%d:ctlb is NULL\n",
			__func__, __LINE__);
		return -1;
	}

	if (channel < 0) {
		CCCI_ERR_INF(0, "cci", "%s:channel=%d is invalid", __func__, channel);
		return -1;
	}
	...
	has_write = snprintf(dev->name, 16, "ccmni%d", channel);
	if (has_write <= 0 || has_write >= 16) {
		CCCI_ERR_INF(md_id, "net", "%s:%d:snprintf fail,has_write=%d",
			__func__, __LINE__, has_write);
		ret = -CCCI_ERR_SNPRINTF_FAIL;
	...
	timer_setup(&ccmni->timer, timer_func, 0);
	ccmni->timer.expires = 0;
...
{
	struct ccmni_v2_ctl_block_t *ctl_b = NULL;
	struct ccmni_v2_instance_t *ccmni = NULL;
	...
	if (channel < 0) {
		CCCI_ERR_INF(0, "net", "%s:channel=%d is invalid", __func__, channel);
		return;
	}

	ccmni = ctl_b->ccmni_v2_instance[channel];
	if (ccmni != NULL) {
		ccmni->ready = 0;
...
int ccmni_v2_init(int md_id)
{
	...
	has_write = snprintf(ctl_b->wakelock_name, sizeof(ctl_b->wakelock_name),
		 "ccci%d_net_v2", (md_id + 1));
	if (has_write <= 0 || has_write >= sizeof(ctl_b->wakelock_name)) {
		CCCI_ERR_INF(md_id, "net", "%s:%d:snprintf fail,has_write=%d",
			__func__, __LINE__, has_write);
		ret = -CCCI_ERR_SNPRINTF_FAIL;
		goto _CCMNI_INSTANCE_CREATE_FAIL;
	...
	ctl_b->ccmni_wake_lock = wakeup_source_register(NULL, ctl_b->wakelock_name);
...
void ccmni_v2_exit(int md_id)
{
	...
	if (ctl_b) {
		...
		wakeup_source_unregister(ctl_b->ccmni_wake_lock);
	}
}
