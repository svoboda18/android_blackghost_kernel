...
void ccci_fs_resetfifo(int md_id)
{
	...
	struct fs_ctl_block_t *ctl_b = NULL;
	unsigned long flag;

	CHECK_MD_ID_RETURN_VOID(md_id);
	ctl_b = fs_ctl_block[md_id];
	if (!ctl_b) {
		CCCI_ERR_INF(md_id, "fs ", "%s:%d:ctlb is NULL\n",
			__func__, __LINE__);
		return;
	}
...
int ccci_fs_init(int md_id)
{
	...
	has_write = snprintf(ctl_b->fs_wakelock_name, sizeof(ctl_b->fs_wakelock_name),
		 "ccci%d_fs", (md_id + 1));
	if (has_write <= 0 || has_write >= sizeof(ctl_b->fs_wakelock_name)) {
		CCCI_ERR_INF(md_id, "fs ", "%s:%d:snprintf fail,has_write=%d",
	...
	ctl_b->fs_wake_lock = wakeup_source_register(NULL, ctl_b->fs_wakelock_name);
	ret =
...
void ccci_fs_exit(int md_id)
{
	...
	wakeup_source_unregister(ctl_b->fs_wake_lock);
	kfree(ctl_b);
	fs_ctl_block[md_id] = NULL;
}
