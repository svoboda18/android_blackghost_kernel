...
{
	unsigned int i, rx, ch, has_write = 0;
	...
	if (!ctlb) {
		CCCI_ERR_INF(md_id, "cci", "%s:%d:ctlb is NULL\n",
			__func__, __LINE__);
		return -1;
	}
			...
			if ((rx_ch[i][0] >= 0)
				...
				channel_name =
				    logic_ch_static_info_tab[rx_ch[i]
							     [0]].m_ch_name;
				has_write = snprintf(str, sizeof(str), "%s(%d,%d) ", channel_name,
					rx_ch[i][0], rx_ch[i][1]);
			} else
				has_write = snprintf(str, sizeof(str), "%s(%d,%d) ", "unknown",
					rx_ch[i][0], rx_ch[i][1]);
			if (has_write <= 0 || has_write >= sizeof(str))
...
int ccci_logic_ctlb_init(int md_id)
{
	...
	/* ctl_b->m_privilege = MD_BOOT_STAGE_0; */
	ctl_b->m_md_id = md_id;
	has_write = snprintf(ctl_b->m_wakelock_name, sizeof(ctl_b->m_wakelock_name),
		 "ccci%d_logic", (md_id + 1));
 	if (has_write <= 0 || has_write >= sizeof(ctl_b->m_wakelock_name)) {
	...
	ctl_b->m_wakeup_wake_lock = wakeup_source_register(NULL, ctl_b->m_wakelock_name);
...
void ccci_logic_ctlb_deinit(int md_id)
{
	...
	if (ctl_b != NULL) {
		...
		__pm_relax(ctl_b->m_wakeup_wake_lock);
		wakeup_source_unregister(ctl_b->m_wakeup_wake_lock);
