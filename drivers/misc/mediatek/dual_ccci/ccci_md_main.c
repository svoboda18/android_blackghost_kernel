...
void ccci_sys_rx_dispatch_cb(void *private)
{
	...
	while (get_logic_ch_data(ch_info, &msg) > 0)
...
void ccci_ee_info_dump(int md_id, struct DEBUG_INFO_T *debug_info)
{
	...
	CCCI_MSG_INF(md_id, "cci", "exception type(%d):%s\n", debug_info->type,
		debug_info->name ? debug_info->name : "Unknown");
	...
	case MD_EX_TYPE_ASSERT:
		...
		has_write = snprintf(ex_info, EE_BUF_LEN,
			 "\n[%s] file:%s line:%d\np1:0x%08x\np2:0x%08x\np3:0x%08x\n",
			 ...
			 debug_info->assert.parameters[1],
			 debug_info->assert.parameters[2]);
		if (has_write <= 0 || has_write >= EE_BUF_LEN)
			CCCI_ERR_INF(md_id, "cci", "%s:%d:snprintf fail,has_write=%d",
	...
	case MD_EE_CASE_ONLY_EX:
		len = strlen(ex_info);
		has_write = snprintf(&ex_info[len], EE_BUF_LEN - len, "\nTime out case\n");
		if (has_write <= 0 || has_write >= EE_BUF_LEN - len)
			CCCI_ERR_INF(md_id, "cci", "%s:%d:snprintf fail,has_write=%d",
				__func__, __LINE__, has_write);
		break;
	...
	case MD_EE_CASE_AP_MASK_I_BIT_TOO_LONG:
		...
		has_write = snprintf(&i_bit_ex_info[len], EE_BUF_LEN - len, "%s", ex_info);
		if (has_write <= 0 || has_write >= EE_BUF_LEN - len)
			CCCI_ERR_INF(md_id, "cci", "%s:%d:snprintf fail,has_write=%d",
				__func__, __LINE__, has_write);
		has_write = snprintf(ex_info, EE_BUF_LEN, "%s", i_bit_ex_info);
		if (has_write <= 0 || has_write >= EE_BUF_LEN)
			CCCI_ERR_INF(md_id, "cci", "%s:%d:snprintf fail,has_write=%d",
				__func__, __LINE__, has_write);
...
void config_misc_info(int md_id, unsigned int base[], unsigned int size)
{
	...
	has_write = snprintf((char *)str, 2 * sizeof(int), "CCIF");
	if (has_write <= 0 || has_write >= 2 * sizeof(int))
		CCCI_ERR_INF(md_id, "ctl", "%s:%d:snprintf fail,has_write=%d",
			__func__, __LINE__, has_write);
...
static void ccci_md_exception(int md_id, struct DEBUG_INFO_T *debug_info)
{
	...
	case MD_EX_TYPE_ASSERT:
		...
		has_write = snprintf(debug_info->assert.file_name,
			 sizeof(debug_info->assert.file_name),
			 ex_info->content.assert.filename);
		if (has_write <= 0 || has_write >= sizeof(debug_info->assert.file_name))
			CCCI_ERR_INF(md_id, "ctl", "%s:%d:snprintf fail,has_write=%d",
				__func__, __LINE__, has_write);
		debug_info->assert.line_num =
	...
	case DSP_EX_TYPE_ASSERT:
		...
		has_write = snprintf(debug_info->dsp_assert.file_name,
			 sizeof(debug_info->dsp_assert.file_name),
			 ex_info->content.assert.filename);
		if (has_write <= 0 || has_write >= sizeof(debug_info->dsp_assert.file_name))
			CCCI_ERR_INF(md_id, "ctl", "%s:%d:snprintf fail,has_write=%d",
				__func__, __LINE__, has_write);
		debug_info->dsp_assert.line_num =
		    ex_info->content.assert.linenumber;
		has_write = snprintf(debug_info->dsp_assert.execution_unit,
			 sizeof(debug_info->dsp_assert.execution_unit),
			 ex_info->envinfo.execution_unit);
		if (has_write <= 0 || has_write >= sizeof(debug_info->dsp_assert.execution_unit))
			CCCI_ERR_INF(md_id, "ctl", "%s:%d:snprintf fail,has_write=%d",
				__func__, __LINE__, has_write);
		debug_info->dsp_assert.parameters[0] =
		    ex_info->content.assert.parameters[0];
	...
	case DSP_EX_TYPE_EXCEPTION:
		...
		has_write = snprintf(debug_info->dsp_exception.execution_unit,
			 sizeof(debug_info->dsp_exception.execution_unit),
			 ex_info->envinfo.execution_unit);
		if (has_write <= 0 || has_write >= sizeof(debug_info->dsp_exception.execution_unit))
			CCCI_ERR_INF(md_id, "ctl", "%s:%d:snprintf fail,has_write=%d",
				__func__, __LINE__, has_write);
		debug_info->dsp_exception.code1 =
		    ex_info->content.fatalerr.error_code.code1;
	...
	case DSP_EX_FATAL_ERROR:
		...
		has_write = snprintf(debug_info->dsp_fatal_err.execution_unit,
			 sizeof(debug_info->dsp_fatal_err.execution_unit),
			 ex_info->envinfo.execution_unit);
		if (has_write <= 0 || has_write >= sizeof(debug_info->dsp_fatal_err.execution_unit))
			CCCI_ERR_INF(md_id, "ctl", "%s:%d:snprintf fail,has_write=%d",
				__func__, __LINE__, has_write);
		debug_info->dsp_fatal_err.err_code[0] =
		    ex_info->content.fatalerr.error_code.code1;
...
static void ex_monitor_func(struct timer_list *t)
{
	...
	struct DEBUG_INFO_T debug_info = {0};
...
#define AED_STR_LEN        (512)
	...
	has_write = snprintf(buff, AED_STR_LEN, "md%d:%s%s", md_id + 1, aed_str, img_inf);
	if (has_write <= 0 || has_write >= AED_STR_LEN)
		CCCI_ERR_INF(md_id, "cci", "%s:%d:snprintf fail,has_write=%d",
			__func__, __LINE__, has_write);
...
{
	struct md_ctl_block_t *ctl_b = NULL;
	struct modem_exception_exp_t *exp_ee_info = NULL;

	CHECK_MD_ID_RETURN_VOID(md_id);
	ctl_b = md_ctlb[md_id];
	if (!ctl_b) {
		CCCI_ERR_INF(md_id, "ctl", "%s:%d:ctlb is NULL\n",
			__func__, __LINE__);
		return;
	}
...
int ccci_start_modem(int md_id)
{
	...
	if (ret == 0)
		...
		//ccci_start_bootup_timer(md_id, 3);
	else
...
int legacy_boot_md_show(int md_id, char *buf, int size)
{
	...
	if (!get_modem_is_enabled(md_id)) {
		has_write = snprintf(buf, size, "md%d:n/a", (md_id + 1));
		if (has_write <= 0 || has_write >= size)
			CCCI_ERR_INF(md_id, "cci", "%s:%d:snprintf fail,has_write=%d",
				__func__, __LINE__, has_write);
	} else {
		has_write = snprintf(buf, size, "md%d:%d", (md_id + 1),
...
int ccci_md_ctrl_init(int md_id)
{
	...
	has_write = snprintf(ctlb->wakelock_name, sizeof(ctlb->wakelock_name), "ccci%d_trm",
		 (md_id + 1));
	if (has_write <= 0 || has_write >= sizeof(ctlb->wakelock_name)) {
		CCCI_ERR_INF(md_id, "ctl", "%s:%d:snprintf fail,has_write=%d",
			__func__, __LINE__, has_write);
		ret = -CCCI_ERR_GET_MEM_LAYOUT_FAIL;
		goto _GET_SMEM_SETTING_FAIL;
	...
	ctlb->trm_wake_lock = wakeup_source_register(NULL, ctlb->wakelock_name);
	...
	ctlb->md_boot_up_check_timer.expires = 0;
...
{
	struct md_ctl_block_t *ctlb = NULL;
	...
	wakeup_source_unregister(ctlb->trm_wake_lock);
