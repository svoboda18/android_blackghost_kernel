...
int ccci_ipc_ipo_h_restore(int md_id)
{
	...
	if (!ctl_b) {
		CCCI_ERR_INF(md_id, "ipc", "%s:%d:ctlb is NULL\n",
			__func__, __LINE__);
		return -1;
	}
			    ...
			    int *len)
{
	...
	int real_size = 0;
	unsigned long flag;
	struct ipc_ctl_block_t *ctl_b = NULL;
		      ...
		      IPC_TASK *task, BUFF *buff_rd)
{
	...
	ilm = (ipc_ilm_t *) ((uint32) ctl_b->ipc_mem +
...
int ccci_ipc_init(int md_id)
{
	...
	has_write = snprintf(buf, 16, "CCCI_IPC_DEV%d", md_id);
	if (has_write <= 0 || has_write >= 16) {
		CCCI_ERR_INF(md_id, "ipc", "%s:%d:snprintf fail,has_write=%d",
