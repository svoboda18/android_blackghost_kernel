...
#define CHECK_MD_ID_RETURN_INT(md_id) \
	do { \
		if (md_id != MD_SYS1) { \
			CCCI_ERR_INF(0, "cci", "%s:md_id=%d is invalid", __func__, md_id); \
			return -1; \
		} \
	} while (0)

#define CHECK_MD_ID_RETURN_POINTER(md_id) \
	do { \
		if (md_id != MD_SYS1) { \
			CCCI_ERR_INF(0, "cci", "%s:md_id=%d is invalid", __func__, md_id); \
			return NULL; \
...
#define CHECK_MD_ID_RETURN_VOID(md_id) \
			do { \
				if (md_id != MD_SYS1) { \
					CCCI_ERR_INF(0, "cci", "%s:md_id=%d is invalid", __func__, md_id); \
					return; \
