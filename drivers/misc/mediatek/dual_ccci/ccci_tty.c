...
#define DUMP_BUF_SIZE 32
	...
	for (i = 0, j = 0; i < len && i < DUMP_BUF_SIZE && j + 4 < DUMP_BUF_SIZE; i++) {
		if (((char_ptr[i] >= 32) && (char_ptr[i] <= 126))) {
			...
			has_write = snprintf(buf+j, DUMP_BUF_SIZE - j, "%s", replace_str);
			j += 2;
		} else {
			has_write = snprintf(buf+j, DUMP_BUF_SIZE - j, "[%02X]", char_ptr[i]);
			j += 4;
		}
		if (has_write < 0)
			CCCI_ERR_INF(md_id, "tty", "%s:%d:snprintf fail,has_write=%d",
...
void ccci_tty_instance_init(struct tty_instance_t *instance)
{
	...
	instance->wake_lock = wakeup_source_register(NULL, instance->wakelock_name);
...
int ccci_tty_init(int md_id)
{
	...
	has_write = snprintf(name, 16, "%s%d", CCCI_TTY_NAME, md_id);
	...
	has_write = snprintf(ctlb->drv_name, 32, CCCI_TTY_NAME "_%d", md_id);
	...
	has_write = snprintf(ctlb->node_name, 16, "ttyC");
	if (has_write <= 0 || has_write >= 16) {
		CCCI_ERR_INF(md_id, "tty", "%s:%d:snprintf fail,has_write=%d",
			__func__, __LINE__, has_write);
		ret = -1;
...
void ccci_tty_exit(int md_id)
{
	...
	if (ctlb != NULL) {
		...
		wakeup_source_unregister(ctlb->ccci_tty_modem.wake_lock);
		wakeup_source_unregister(ctlb->ccci_tty_meta.wake_lock);
		wakeup_source_unregister(ctlb->ccci_tty_ipc.wake_lock);
#ifdef CONFIG_MTK_ICUSB_SUPPORT
