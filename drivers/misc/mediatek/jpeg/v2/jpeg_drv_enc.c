		     ...
		     dma_addr_t *mva, unsigned int *size, int port)
{
	...
	if (ion_kernel_ioctl(client, ION_CMD_MULTIMEDIA,
		...
		JPEG_MSG("jpeg_ion_get_mva: config buffer failed.%p -%p\n",
			client, handle);
	 ...
	 int srcFd, int srcFd2)
{
	...
	if (yuv_format == YUYV || yuv_format == YVYU)
	{
		...
		if (0 != jpeg_ion_get_mva(pIonClient, handle, &srcAddr, &bufSize, 0))
			return 0;
	...
	else
	{
		...
		if (0 != jpeg_ion_get_mva(pIonClient, handle, &srcAddr_C, &bufSize, 0))
			return 0;
		 ...
		 unsigned int init_offset, unsigned int offset_mask)
{
	...
	if (0 != jpeg_ion_get_mva(pIonClient, handle, &dst_addr, &bufSize, 0))
		return 0;
