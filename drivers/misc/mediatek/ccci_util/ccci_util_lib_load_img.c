...
#define MAX_REMAP_SIZE (1024 * 1024)
	...
	/*  Gen MD image name */
	if (img->type == IMG_MD) {
		ret = snprintf(img_name, IMG_NAME_LEN,
			"modem_%s.img", post_fix);
		if (ret < 0 || ret >= IMG_NAME_LEN) {
			CCCI_UTIL_ERR_MSG_WITH_ID(md_id,
				"[Error] snprintf return error code :%d\n",
				ret);
...
TRY_LOAD_IMG:
	...
	img->file_name[sizeof(img->file_name)-1] = '\0';
	...
	if (img->type == IMG_MD) {
		ret_len = snprintf(img_str, sizeof(md_img_info_str[md_id]),
			"MD:%s*%s*%s*%s*%s\nAP:%s*%s*%08x (MD)%08x\n",
		...
		if (ret_len < 0 || ret_len >= sizeof(md_img_info_str[md_id]))
			CCCI_UTIL_ERR_MSG_WITH_ID(md_id,
				"[Error] 0 snprintf return error: %d\n", ret_len);
	}
 ...
 out:
	...
	if (img_err_str != NULL) {
		if (ret == -CCCI_ERR_LOAD_IMG_SIGN_FAIL) {
			ret_len = snprintf(img_err_str, IMG_ERR_STR_LEN,
				"%s Signature check fail\n", img->file_name);
			if (ret_len < 0 || ret_len >= IMG_ERR_STR_LEN)
				CCCI_UTIL_ERR_MSG_WITH_ID(md_id,
					"[Error] 1 snprintf return error: %d\n", ret_len);
			...
			if (ret_len < 0 || ret_len >= IMG_ERR_STR_LEN)
				CCCI_UTIL_ERR_MSG_WITH_ID(md_id,
					"[Error] 2 snprintf return error: %d\n", ret_len);
			...
			if (ret_len < 0 || ret_len >= IMG_ERR_STR_LEN)
				CCCI_UTIL_ERR_MSG_WITH_ID(md_id,
					"[Error] 3 snprintf return error: %d\n", ret_len);
			...
			if (ret_len < 0 || ret_len >= IMG_ERR_STR_LEN)
				CCCI_UTIL_ERR_MSG_WITH_ID(md_id,
					"[Error] 4 snprintf return error: %d\n", ret_len);
		}
	}
