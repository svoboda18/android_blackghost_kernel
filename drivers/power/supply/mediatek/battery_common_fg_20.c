...
#if 0 /* commit this since POWER_SUPPLY_TYPE_WIRELESS is not defined */
...
void mt_battery_update_time(struct timespec *pre_time, BATTERY_TIME_ENUM duration_type)
{
	...
	if (duration_type >= 0)
		battery_duration_time[duration_type] =
			timespec_sub(time, *pre_time);
	...
	if (duration_type >= 0)
		...
		battery_log(BAT_LOG_FULL,
		    "[Battery] mt_battery_update_duration_time , last_time=%d current_time=%d type=%d\n",
		    (int)pre_time->tv_sec, (int)time.tv_sec, duration_type);
...
struct timespec mt_battery_get_duration_time_act(BATTERY_TIME_ENUM duration_type)
{
	if (duration_type >= 0)
		...
		return battery_duration_time[0];
}
				...
				enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_TIME_TO_FULL_NOW:
		...
		if ((ret == POWER_SUPPLY_CAPACITY_LEVEL_FULL) ||
			...
			int q_max_now = battery_meter_get_QMAX25();
			int remain_ui = 100 - data->BAT_CAPACITY;
			int remain_mah = remain_ui * q_max_now / 10;
			int time_to_full = 0;

			if (data->BAT_CURRENT_NOW > 0)
				time_to_full = remain_mah * 360 / data->BAT_CURRENT_NOW;
	...
	case POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN:
		if (check_cap_level(data->BAT_CAPACITY) ==
			...
			q_max_mah = battery_meter_get_QMAX25();

			q_max_uah = q_max_mah * 1000;
			if (q_max_uah <= 100000) {
				battery_log(BAT_LOG_CRTI, "[%s] q_max_mah:%d q_max_uah:%d\n",
...
#if 0 /* comment this since POWER_SUPPLY_TYPE_WIRELESS is not defined */
...
bool gend_bat_update_thread = false;

int bat_update_thread(void *x)
{
	/* Run on a process content */
	while (gend_bat_update_thread == false) {
...
static int battery_probe(struct platform_device *dev)
{
	...
	battery_suspend_lock = wakeup_source_register(NULL, "battery suspend wakelock");
	battery_meter_lock = wakeup_source_register(NULL, "battery meter wakelock");
#if defined(CONFIG_MTK_PUMP_EXPRESS_SUPPORT)
