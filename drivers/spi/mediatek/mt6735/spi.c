...
static int mt_spi_probe(struct platform_device *pdev)
{
	...
	ms->cur_transfer = NULL;
	ms->next_transfer = NULL;
	ms->wk_lock = wakeup_source_register(NULL, "spi_wakelock");
	...
	SPI_INFO(&pdev->dev, "zxd: ready to register spi master\n");
	ret = spi_register_master(master);
	if (ret) {
		SPI_INFO(&pdev->dev, "zxd: register spi master failed!\n");
		dev_err(&pdev->dev, "spi_register_master fails.\n");
		goto out_free;
	} else {

		SPI_INFO(&pdev->dev, "zxd: register spi master success!\n");
#ifdef SPI_TRUSTKERNEL_TEE_SUPPORT
		SPI_INFO(&pdev->dev, "zxd: ready to call tee_clkmgr_register0(pdev->id = %d)\n", pdev->id);
		tee_clkmgr_register1("spi", pdev->id,
				enable_clk, disable_clk, (void *) ms);
#endif
